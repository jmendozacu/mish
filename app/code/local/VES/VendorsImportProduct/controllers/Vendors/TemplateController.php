...
class VES_VendorsImportProduct_Vendors_TemplateController extends VES_Vendors_Controller_Action
{
    ...
    public function indexAction ()
    {
        ...
        $this->_addContent($this->getLayout()
            ->createBlock('helpdesk/vendors_template'));
        $this->renderLayout();
    }
    ...
    public function addAction ()
    {
        ...
        $this->_addBreadcrumb(Mage::helper('adminhtml')->__('Template  Manager'),
                Mage::helper('adminhtml')->__('Template Manager'), mage::getUrl('*/*/'));
        $this->_addBreadcrumb(Mage::helper('adminhtml')->__('Add Template '), Mage::helper('adminhtml')->__('Add Template'));

        $this->getLayout()
            ->getBlock('head')
            ->setCanLoadExtJs(true);
        $this->_addContent($this->getLayout()->createBlock('helpdesk/vendors_template_edit'));
    ...
    public function editAction ()
    {
        ...
        if ($template->getId()) {
            ...
            $this->_addBreadcrumb(Mage::helper('adminhtml')->__('Quick Responses'),
                    Mage::helper('adminhtml')->__('Quick Responses'), mage::getUrl('*/*/'));
            ...
            Mage::getSingleton('vendors/session')->addError(Mage::helper('adminhtml')->__('The Template does not exist.'));
            $this->_redirect('*/*/');
        }
    }
    ...
    public function saveAction ()
    {
        if ($data = $this->getRequest()->getPost()) {
            ...
            try {
                ...
                Mage::getSingleton('vendors/session')->addSuccess(Mage::helper('adminhtml')->__('Template was successfully saved'));
        ...
        Mage::getSingleton('vendors/session')->addError(Mage::helper('adminhtml')->__('Unable to find Template to save'));
        $this->_redirect('*/*/');
    }
    ...
    {
        $idss = $this->getRequest()->getParam('template_id');
        $ids=explode(',', $idss);
        if (!is_array($ids)) {
            Mage::getSingleton('vendors/session')->addError(Mage::helper('adminhtml')->__('Please select Template(s)'));
        } else {
            try {
