...
class VES_VendorsInventory_Block_Purchaseorder_Edit_Tab_Products extends Mage_Adminhtml_Block_Widget_Grid {
    ...
    public function __construct() {
        ...
        if (($this->getPurchaseOrder() && $this->getPurchaseOrder()->getId()) || Mage::getModel('vendors/session')->getData('purchaseorder_product_import')) {
        ...
        if (isset($warehouseIds)) {
            ...
            foreach ($warehouseIds as $warehouseId) {
                if (isset($warehouse_id)) {
                    if (!Mage::helper('vendorsinventory/adjuststock')->getPermission($warehouse_id, 'can_purchase_product')) {
                        $disableCheck = true;
                    }
                }
            }
    ...
    protected function _prepareColumns() {
        ...
        if ($this->_editable) {
            ...
            $this->addColumn('product_name', array(
                ...
                'align' => 'left',
                'index' => 'product_name',
                'renderer' => 'vendorsinventory/purchaseorder_edit_tab_renderer_product',
            ));
        }
        ...
        if ($warehouseIds = $this->getRequest()->getParam('warehouse_ids')) {
            ...
            foreach ($warehouseIds as $warehouseId) {
                $this->addColumn('warehouse_' . $warehouseId, array(
                    ...
                    'align' => 'right',
                    'sortable' => false,
                    'renderer' => 'vendorsinventory/purchaseorder_edit_tab_renderer_AvailableQty'
                ));
            }
            ...
            if (count($results) > 0) {
                foreach ($results as $result) {
                    $neweditable = true;
                    if (!Mage::helper('vendorsinventory/adjuststock')->getPermission($result['warehouse_id'], 'can_purchase_product') || !$this->_editable) {
                        $neweditable = false;
                    }
                    $this->addColumn('warehouse_' . $result['warehouse_id'], array(
                        ...
                        'sortable' => false,
                        'renderer' => 'vendorsinventory/purchaseorder_edit_tab_renderer_warehouse'
                    ));
                }
            } else {
                $purchaseOrder = $this->getPurchaseOrder();
    ...
    protected function _getSelectedProducts() {
        ...
        if (!is_array($products) || Mage::getModel('vendors/session')->getData('purchaseorder_product_import')) {
    ...
    public function getSelectedRelatedProducts() {
        ...
        if ($purchaseOrderProductImports = Mage::getModel('vendors/session')->getData('purchaseorder_product_import')) {
