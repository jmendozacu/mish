 ...
 */
class VES_VendorsInventory_Block_Purchaseorder_Edit_Tab_Delivery extends Mage_Adminhtml_Block_Widget_Grid {
    ...
    protected function _prepareLayout() {
        if ($purchaseOrderId = $this->getRequest()->getParam('id')) {
            ...
            if ($pStatus != Magestore_Inventorypurchasing_Model_Purchaseorder::PENDING_STATUS && $results && (count($results) >= 1)) {
                if ($this->checkCreateAllDelivery() && ($pStatus == Magestore_Inventorypurchasing_Model_Purchaseorder::AWAITING_DELIVERY_STATUS || $pStatus == Magestore_Inventorypurchasing_Model_Purchaseorder::RECEIVING_STATUS)
                ) {
                ...
                if ($this->checkCreateNewDelivery() && ($pStatus == Magestore_Inventorypurchasing_Model_Purchaseorder::AWAITING_DELIVERY_STATUS || $pStatus == Magestore_Inventorypurchasing_Model_Purchaseorder::RECEIVING_STATUS)
                ) {
        ...
        if ($this->getRequest()->getParam('id')) {
            ...
            foreach ($results as $result) {
                $this->addColumn('warehouse_' . $result['warehouse_id'], array(
                    ...
                    'align' => 'right',
                    'sortable' => false,
                    'renderer' => 'vendorsinventory/purchaseorder_edit_tab_renderer_delivery_warehouse'
                ));
            }
        }
