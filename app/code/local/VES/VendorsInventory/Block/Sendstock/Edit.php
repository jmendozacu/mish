...
class VES_Vendorsinventory_Block_Sendstock_Edit extends Mage_Adminhtml_Block_Widget_Form_Container {
    ...
    public function __construct() {
        ...
        if (!$id) {
            if ($source && $target) {
                $this->_updateButton('back', 'onclick', 'setLocation(\'' . $this->getUrl("vendors/inventory_sendstock/new") . '\')');
            ...
            $this->_updateButton('back', 'onclick', 'setLocation(\'' . $this->getUrl("vendors/inventory_sendstock/index") . '\')');
            $this->removeButton('save');
            $this->removeButton('delete');
            $this->removeButton('reset');
            ...
            $this->_addButton('print', array(
                ...
                'onclick' => 'setLocation(\'' . $this->getUrl('*/*/exportpdfsendstock', array('_current' => true)) . '\')',
                'class' => 'save'
            ), -100);
        ...
        $this->_formScripts[] = "
            ...
            function saveAndContinueEditCheck(){
                if(confirm('".Mage::helper('inventorywarehouse')->__('Are you sure you want to create this stock sending?')."')){
                    ...
                    if((!checkProduct) || (checkProduct=='')){
                        ...
                        var check_product_url = '" . $this->getUrl('vendors/inventory_sendstock/checkproduct') . "';
            ...
            function cancelSending(){
                if (confirm('Are you sure you want to cancel sending stock? The Qty. sent will be instantly added back to the total Qty. in your source warehouse.')) { 
                    var url = '" . $this->getUrl('vendors/inventory_sendstock/cancel', array('id' => $this->getRequest()->getParam('id'))) . "';
                    if($('loading-mask')){
                        $('loading-mask').style.display = 'block';
                        $('loading-mask').style.height = '900px';                    
                        $('loading-mask').style.width = '1500px';                    
            ...
            function uploadFile() {
                ...
                xhr.open('POST', '" . $this->getUrl('vendors/inventory_sendstock/getImportCsv', array('source' => $source, 'target' => $target))."reason/'+reason);
                xhr.send(fd);
           }
           ...
           function uploadComplete(evt) {
                var import_data = '" . Mage::getModel('vendors/session')->getData('null_sendstock_product_import') . "';           
                if(import_data == '1'){
