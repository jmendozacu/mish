 ...
 */
class VES_VendorsInventory_Block_Requeststock_Edit_Tab_Form extends Mage_Adminhtml_Block_Widget_Form {
    ...
    protected function _prepareForm() {
        ...
        $this->setForm($form);
        $data = '';
        if (Mage::getSingleton('vendors/session')->getRequestStockData()) {
            $data = Mage::getSingleton('vendors/session')->getRequestStockData();
            Mage::getSingleton('vendors/session')->getRequestStockData(null);
        ...
        $fieldset = $form->addFieldset('requeststock_form', array(
            'legend' => Mage::helper('vendorsinventory')->__('Stock Request Information')
        ));
        ...
        if ($this->getRequest()->getParam('id')) {
            ...
            $sourceOptions = Mage::helper('vendorsinventory/requeststock')->getWarehouseByAdmin();
            $targetOptions = Mage::helper('vendorsinventory/requeststock')->getAllWarehouseRequeststock();
        }
        if (count($sourceOptions) == 0) {
            $fieldset->addField('continue_button', 'note', array(
                'label' => Mage::helper('inventoryplus')->__('Destination'),
                'class' => 'required-entry',
                'text' => Mage::helper('vendorsinventory')->__('You have no permission to create stock sending'),
            ));
            return parent::_prepareForm();
        }
        if ($source && $target) {
            $fieldset->addField('warehouse_id_from', 'select', array(
                'label' => Mage::helper('vendorsinventory')->__('Source'),
                'class' => 'required-entry',
            ...
            $fieldset->addField('warehouse_id_to', 'select', array(
                'label' => Mage::helper('vendorsinventory')->__('Destination'),
                'class' => 'required-entry',
                        ...
                        <script type="text/javascript">
                            ...
                            function continueTransfer(){
                                if($("warehouse_id_from").value != $("warehouse_id_to").value){
                                    var url = "' . $this->getUrl('vendors/inventory_requeststock/new') . 'source/"+$("warehouse_id_from").value+"/target/"+$("warehouse_id_to").value;
            ...
            $fieldset->addField('reason', 'editor', array(
                'name' => 'reason',
                'label' => Mage::helper('vendorsinventory')->__('Reason(s) for requesting stock'),
                'title' => Mage::helper('vendorsinventory')->__('Reason(s) for requesting stock'),
