 ...
 */
class VES_VendorsInventory_Block_Requeststock_Edit extends Mage_Adminhtml_Block_Widget_Form_Container {
    ...
    public function __construct() {
        ...
        if ($source && $target) {
            $this->_updateButton('back', 'onclick', 'setLocation(\'' . $this->getUrl("vendors/inventory_requeststock/new") . '\')');
        ...
        if ($id) {
            ...
            $this->_addButton('print', array(
                ...
                'onclick' => 'setLocation(\'' . $this->getUrl('*/*/exportpdfrequeststock', array('_current' => true)) . '\')',
                'class' => 'save'
            ), -100);
        }
        ...
        $this->_formScripts[] = "
            ...
            function saveAndContinueEditCheck(){
                ...
                if((!checkProduct) || (checkProduct=='')){
                    ...
                    var check_product_url = '" . $this->getUrl('vendors/inventory_requeststock/checkproduct') . "';
            ...
            function cancelRequesting(){
                if (confirm('Are you sure you want to cancel requesting stock? The Qty. requested will be instantly subtracted from the total Qty. in your destination warehouse.')) {
                    var url = '" . $this->getUrl('vendors/inventory_requeststock/cancel', array('id' => $this->getRequest()->getParam('id'))) . "';
                    if($('loading-mask')){
            ...
            function uploadFile() {
                ...
                var url = '".$this->getUrl('vendors/inventory_requeststock/getImportCsv', array('source' => $source, 'target' => $target))."'+'reason/'+reason;
                xhr.open('POST', url);
                xhr.send(fd);
           }
           ...
           function uploadComplete(evt) {
                var import_data = '" . Mage::getModel('vendors/session')->getData('null_requeststock_product_import') . "';           
                if(import_data == '1'){
    ...
    public function checkCancelRequeststock($id)
    {
        ...
        if($warehouseId && Mage::helper('vendorsinventory')->canTransfer($admin->getId(), $warehouseId)){
