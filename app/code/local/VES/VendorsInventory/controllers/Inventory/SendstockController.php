 ...
 */
class VES_VendorsInventory_Inventory_SendstockController extends VES_Vendors_Controller_Action implements Magestore_Inventoryplus_Controller_Scan {
    ...
    public function newAction() {
        ...
        $this->_addContent($this->getLayout()->createBlock('vendorsinventory/sendstock_edit'))
                ->_addLeft($this->getLayout()->createBlock('vendorsinventory/sendstock_edit_tabs'));
        $this->renderLayout();
    }
    ...
    public function productsAction() {
        ...
        $this->renderLayout();
        if (Mage::getModel('vendors/session')->getData('sendstock_product_import'))
            Mage::getModel('vendors/session')->setData('sendstock_product_import', null);
    }
...
//            print_r(Mage::getModel('vendors/session')->getUser()->getData());die;
            ...
            try {
                ...
                if (Mage::getStoreConfig('inventoryplus/transaction/transaction_notice') == 1) {
                    ...
                    if ($data['warehouse_id_to'] != '' || $data['warehouse_id_to'] != '1') {
                        ...
                        try {
                            if ($warehouseTarget) {
                                Mage::helper('vendorsinventory/sendstock')->sendSendstockEmail($warehouseTarget, $model->getId(), 1, $stockName);
                            }
                        } catch (Exception $e) {
...
//                $this->removeScanData();
                ...
                if ($this->getRequest()->getParam('back')) {
                    $this->_redirect('vendors/inventory_sendstock/edit', array('id' => $model->getId()));
                    return;
                }
                $this->_redirect('*/*/');
                return;
                ...
                $this->_redirect('vendors/inventory_sendstock/edit', array('id' => $this->getRequest()->getParam('id')));
                return;
            }
            ...
            Mage::getSingleton('vendors/session')->addError(Mage::helper('inventoryplus')->__('Unable to save'));
            $this->_redirect('*/*/');
        }
    }
    ...
    public function cancelAction() {
        ...
        try {
            ...
            if ($this->getRequest()->getParam('warehouse_id')) {
                ...
                $this->_redirect('vendors/inventory_sendstock/index');
            }
            return;
        } catch (Exception $e) {
            Mage::getSingleton('vendors/session')->addError($e->getMessage());
    ...
    public function getImportCsvAction() {
        ...
        if (isset($fileToUpload['fileToUpload']['name']) && $fileToUpload['fileToUpload']['name'] != '') {
            try {
                Mage::getModel('vendors/session')->setData('send_stock_reason', null);
                if ($this->getRequest()->getParam('reason')) {
                    Mage::getModel('vendors/session')->setData('send_stock_reason', $this->getRequest()->getParam('reason'));
                }
    ...
    public function exportCsvAction() {
        ...
        $content = $this->getLayout()
                ->createBlock('vendorsinventory/sendstock_grid')
                ->getCsv();
    ...
    public function exportpdfsendstockAction() {
        try {
            ...
            $contents .= $this->getLayout()->createBlock('vendorsinventory/sendstock')
                    ...
                    ->setTemplate('vendorsinventory/sendstock/sendstock.phtml')
                    ->toHtml();
