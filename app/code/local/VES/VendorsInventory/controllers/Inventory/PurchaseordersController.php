 ...
 */
class VES_VendorsInventory_Inventory_PurchaseordersController extends VES_Vendors_Controller_Action {// implements Magestore_Inventoryplus_Controller_Scan {
    /*
     * Menu path
     * 
     * @var string
    ...
    public function editAction() {
        ...
        if ($model->getId() || $purchaseOrderId == 0) {
            ...
            $this->_addContent($this->getLayout()->createBlock('vendorsinventory/purchaseorder_edit'))
                    ->_addLeft($this->getLayout()->createBlock('vendorsinventory/purchaseorder_edit_tabs'));
    ...
    public function newAction() {
        ...
        if (isset($supplier_id) && $supplier_id) {
            ...
            $this->getLayout()->getBlock('head')->setCanLoadExtJs(true);
            $this->_addContent($this->getLayout()->createBlock('vendorsinventory/purchaseorder_new'))
                    ->_addLeft($this->getLayout()->createBlock('vendorsinventory/purchaseorder_new_tabs'));
            $this->renderLayout();
        }
    }
        ...
        if ($data = $this->getRequest()->getPost()) {
            ...
            if ($data['ship_via'] == 'new' && $data['ship_via_new']) {
                try {
                    ...
                    Mage::getSingleton('vendors/session')->addSuccess(Mage::helper('inventorypurchasing')->__('The shipping method "%s" has been created.', $data['ship_via_new']));
                } catch (Exception $e) {
            ...
            if ($data['payment_term'] == 'new' && $data['payment_term_new']) {
                try {
                    ...
                    Mage::getSingleton('vendors/session')->addSuccess(Mage::helper('inventorypurchasing')->__('The payment term "%s" has been created with the default payment period of 7 days.', $data['payment_term_new']));
                } catch (Exception $e) {
                    Mage::getSingleton('vendors/session')->addError($e->getMessage());
                }
            }
            ...
            try {
                if (!Mage::helper('vendorsinventory/purchaseorder')->haveDelivery($this->getRequest()->getParam('id'))) {
...
//                                    $totalAmounts += $codeArr['qty_order'] * $cost * (1 - $discount/100) * (1+ $tax/100);
    ...
    public function massStatusAction() {
        ...
        if (!is_array($purchaseOrderIds)) {
            Mage::getSingleton('vendors/session')->addError(Mage::helper('inventorypurchasing')->__('Please select purchase order(s)!'));
        } else {
    ...
    public function exportCsvAction() {
        ...
        $content = $this->getLayout()
                ->createBlock('vendorsinventory/purchaseorder_grid')
                ->getCsv();
    ...
    public function exportcsvpurchaseorderAction() {
        ...
        $contents .= $this->getLayout()->createBlock('vendorsinventory/purchaseorder')
    ...
    public function productAction() {
        $this->loadLayout();
        $this->getLayout()->getBlock('vendorsinventory.purchaseorder.edit.tab.products')
        ...
        $this->renderLayout();
        if (Mage::getModel('vendors/session')->getData('purchaseorder_product_import')) {
            Mage::getModel('vendors/session')->setData('purchaseorder_product_import', null);
        }
    }
    ...
    public function deliveryAction() {
        $this->loadLayout();
        $this->getLayout()->getBlock('vendorsinventory.purchaseorder.edit.tab.delivery')
    ...
    public function newDeliveryAction() {
        ...
        if ($model->getId() || $purchaseOrderId == 0) {
            ...
            $this->getLayout()->getBlock('head')->setCanLoadExtJs(true);
            $this->_addContent($this->getLayout()->createBlock('vendorsinventory/purchaseorder_editdelivery'))
                    ->_addLeft($this->getLayout()->createBlock('vendorsinventory/purchaseorder_editdelivery_tabs'));
            $this->renderLayout();

            if (Mage::getModel('vendors/session')->getData('delivery_purchaseorder_product_import')) {
                Mage::getModel('vendors/session')->setData('delivery_purchaseorder_product_import', null);
            }
        } else {
            Mage::getSingleton('vendors/session')->addError(
    ...
    public function prepareDeliveryAction() {
        $this->_title($this->__('Inventory'))
                ->_title($this->__('Add New Delivery'));
        $this->loadLayout();
        $this->getLayout()->getBlock('vendorsinventory.purchaseorder.edit.tab.preparedelivery')
            ...
            if ($data = $this->getRequest()->getPost()) {
                ...
                if (isset($data['delivery_products'])) {
                    ...
                    if (count($deliveryProducts)) {
                        ...
                        Mage::getModel('vendors/session')->unsetData('delivery_create_at');
                ...
                Mage::getSingleton('vendors/session')->setFormData(false);
                $this->_redirect('vendors/inventory_purchaseorders/edit', array('id' => $this->getRequest()->getParam('purchaseorder_id'), 'active' => 'delivery'));
                return;
            }
        } catch (Exception $e) {
            Mage::getSingleton('vendors/session')->addError($e->getMessage());
            ...
            if ($data = $this->getRequest()->getPost()) {
                ...
                if (isset($data['returnorder_products'])) {
                    ...
                    if (count($returnorderProducts)) {
                        $productIds = '';
                        $supplierId = Mage::helper('vendorsinventory/purchaseorder')->getDataByPurchaseOrderId($purchaseOrderId, 'supplier_id');
                        ...
                        Mage::getModel('vendors/session')->unsetData('returnorder_create_at');
                        Mage::getModel('vendors/session')->unsetData('returnorder_reason');
                    } else {
                        Mage::getSingleton('vendors/session')->addError(
                ...
                Mage::getSingleton('vendors/session')->setFormData(false);
                $this->_redirect('vendors/inventory_purchaseorders/edit', array('id' => $this->getRequest()->getParam('purchaseorder_id'), 'active' => 'return'));
                return;
            }
        } catch (Exception $e) {
            Mage::getSingleton('vendors/session')->addError($e->getMessage());
    ...
    public function returnOrderAction() {
        $this->loadLayout();
        $this->getLayout()->getBlock('vendorsinventory.purchaseorder.edit.tab.returnorder');
        $this->renderLayout();
    }
    ...
    public function newReturnOrderAction() {
        ...
        if ($model->getId() || $purchaseOrderId == 0) {
            ...
            $this->getLayout()->getBlock('head')->setCanLoadExtJs(true);
            $this->_addContent($this->getLayout()->createBlock('vendorsinventory/purchaseorder_returnorder'))
                    ->_addLeft($this->getLayout()->createBlock('vendorsinventory/purchaseorder_returnorder_tabs'));

            $this->renderLayout();
            if (Mage::getModel('vendors/session')->getData('returnorder_product_import')) {
                Mage::getModel('vendors/session')->setData('returnorder_product_import', null);
            }
        } else {
            Mage::getSingleton('vendors/session')->addError(
                    Mage::helper('inventorypurchasing')->__('Item does not exist')
    ...
    public function prepareNewReturnOrderAction() {
        $this->_title($this->__('Inventory'))
                ->_title($this->__('Add New Return Order'));
        $this->loadLayout();
        $this->getLayout()->getBlock('vendorsinventory.purchaseorder.edit.tab.preparenewreturnorder')
                ->setProducts($this->getRequest()->getPost('isproducts', null));
...
//                    $returnOrderProduct->delete();
                ...
                Mage::getSingleton('vendors/session')->setFormData(false);
                return $this->_redirect('vendors/inventory_purchaseorders/edit', array('id' => $this->getRequest()->getParam('purchaseorder_id'), 'active' => 'return'));
            } else {
                Mage::getSingleton('vendors/session')->addError(
                        Mage::helper('inventorypurchasing')->__('Unable to find return order to save!')
                );
    ...
    public function importproductAction() {
        if (isset($_FILES['fileToUpload']['name']) && $_FILES['fileToUpload']['name'] != '') {
            try {
                ...
                $fields = array();
                $count = 0;
                $purchaseorderHelper = Mage::helper('vendorsinventory/purchaseorder');
                if (count($dataFile))
    ...
    public function importproductforcreatedeliveryAction() {
        ...
        Mage::getModel('vendors/session')->setData('delivery_create_at', null);
        if (isset($getParams['create_at'])) {
            Mage::getModel('vendors/session')->setData('delivery_create_at', $getParams['create_at']);
        }
        $checkField = array(
                                ...
                                foreach ($row as $index => $cell) {
                                    if (isset($fields[$index])) {
                                        if (!in_array($fields[$index], $checkField)) {
                                            ...
                                            if (!Mage::helper('vendorsinventory/adjuststock')->getPermission($warehouse[1], 'can_purchase_product')) {
                                                continue;
                                            }
                                        }
    ...
    public function importproductforreturnorderAction() {
        $getParams = $this->getRequest()->getParams();
        Mage::getModel('vendors/session')->setData('returnorder_create_at', null);
        Mage::getModel('vendors/session')->setData('returnorder_reason', null);
        if (isset($getParams['create_at'])) {
            Mage::getModel('vendors/session')->setData('returnorder_create_at', $getParams['create_at']);
        }
        if (isset($getParams['reason'])) {
            Mage::getModel('vendors/session')->setData('returnorder_reason', $getParams['reason']);
        }
    ...
    public function showhistoryAction() {
        $form_html = $this->getLayout()
                ->createBlock('vendorsinventory/purchaseorder')
    ...
    public function sendEmail($supplierId, $sqlNews, $purchaseOrderId) {
        ...
        if ($supplierId) {
            $supplierInfo = Mage::helper('vendorsinventory/supplier')->getBillingAddressBySupplierId($supplierId);
        }
        if (!$supplierId) {
            $supplierInfo = Mage::helper('vendorsinventory/purchaseorder')->getBilingAddressByPurchaseOrderId($purchaseOrderId);
        }
...
//Infomation - variable in email template (we'll use then send email successfullly)
    ...
    public function productNotReceiveAction() {
        $this->loadLayout();
        $this->getLayout()->getBlock('vendorsinventory.purchaseorder.edit.tab.productnotreceive');
        $this->renderLayout();
    }
...
//        require("lib/Magestore/Pdf/html2fpdf.php");
        ...
        $fileName = 'returned-items-purchase-order-' . $purchaseOrderId . '-' . Mage::helper('core')->formatDate(now(), 'short');
    ...
    public function printreceiptforundersellingAction() {
        ...
        $fileName = 'shortfall-items-purchase-order-' . $purchaseOrderId . '-' . Mage::helper('core')->formatDate(now(), 'short');
