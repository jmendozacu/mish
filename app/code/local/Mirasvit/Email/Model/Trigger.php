    ...
    public function loadArgs($uniqKey, &$args)
    {
        ...
        if (isset($args['quote_id']) && $args['quote_id']) {
            ...
            foreach ($args['quote']->getAllVisibleItems() as $item) {
                $taxPrice   = $item->getCalculationPrice() * $item->getQty() + $item->getBaseTaxAmount();
                $finalPrice = $item->getCalculationPrice() * $item->getQty() + $item->getBaseTaxAmount() + $item->getWeeeTaxAppliedAmount();
                $item->setTaxPrice($taxPrice);
                $item->setFinalPrice($finalPrice);

                $args['item_'.$idx.'_name'] = $item->getName();
                $args['item_'.$idx.'_price'] = $finalPrice;
