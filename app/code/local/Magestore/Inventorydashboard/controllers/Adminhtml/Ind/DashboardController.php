 ...
 */
class Magestore_Inventorydashboard_Adminhtml_Ind_DashboardController extends Magestore_Inventoryplus_Controller_Action {
    ...
    protected function _initAction() {
        $this->loadLayout()
                ->_setActiveMenu('inventoryplus/dashboard')
                ->_addBreadcrumb(
    ...
    protected function _isAllowed() {
        return Mage::getSingleton('admin/session')->isAllowed('inventoryplus/dashboard');
    }
    ...
    public function deletetabAction() {
        if ($tabId = $this->getRequest()->getParam('tab_id')) {
            $model = Mage::getModel('inventorydashboard/tabs')->setId($tabId);
            $model->delete();
            Mage::getSingleton('adminhtml/session')->addSuccess(
    ...
    public function changecharttypeAction() {
        ...
        $chartCodes = $chartReports[$reportCode];
        $i = 1;
        foreach ($chartCodes as $chartCode) {
            ...
            $response .= '<li style="float:left;margin-right:15px">
                <input type="radio" class="radio validate-one-required-by-name validation-passed" value="' . $chartCode . '" name="chart_type" title="' . $chartTitle . '" ' . $checked . '/>
