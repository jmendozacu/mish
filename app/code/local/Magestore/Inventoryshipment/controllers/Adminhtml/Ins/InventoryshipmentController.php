 ...
 */
class Magestore_Inventoryshipment_Adminhtml_Ins_InventoryshipmentController extends Magestore_Inventoryplus_Controller_Action
{
     ...
     * @var string
     */
    protected $_menu_path = 'inventoryplus/sales_shipment';
    ...
    public function gridAction() {
        ...
        $this->getResponse()->setBody(
            $this->getLayout()->createBlock('inventoryshipment/adminhtml_inventoryshipment_grid')->toHtml()
        );
    }
        ...
        if ($data = $this->getRequest()->getPost()) {
            if (isset($FILES['filename']['name']) && $FILES['filename']['name'] != '') {
                try {
                    ...
                    $result = $uploader->save($path, $FILES['filename']['name'] );
        ...
        if (!is_array($inventoryshipmentIds)) {
            ...
            try {
                foreach ($inventoryshipmentIds as $inventoryshipmentId) {
                    $inventoryshipment = Mage::getModel('inventoryshipment/inventoryshipment')->setId($inventoryshipmentId);
    ...
    public function massStatusAction()
    {
        ...
        if (!is_array($inventoryshipmentIds)) {
            ...
            try {
                $collection = Mage::getSingleton('inventoryshipment/inventoryshipment')
                    ->getCollection()->addFieldToFilter('', array('in' => $inventoryshipmentIds));
