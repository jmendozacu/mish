...
class Magestore_Storepickup_Block_Adminhtml_Specialday_Edit extends Mage_Adminhtml_Block_Widget_Form_Container {
    ...
    public function getHeaderText() {
        if (Mage::registry('specialday_data') && Mage::registry('specialday_data')->getId()) {
            return Mage::helper('storepickup')->__("Edit Special Day '%s'", $this->htmlEscape(Mage::registry('specialday_data')->getData('date')));
        } elseif ($this->getRequest()->getParam('id')) {
            return Mage::helper('storepickup')->__("Edit Special Day '%s'", Mage::getModel('storepickup/specialday')->load($this->getRequest()->getParam('id'))->getDate());
        } else {
