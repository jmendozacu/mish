                                                 ...
                                                 $useProductPrice = false)
    {
        ...
        if ($item->getHasChildren() && $item->isShipSeparately()) {
            foreach ($item->getChildren() as $child) {
                ...
                if (!$item->getProduct()->getWeightType()) {//dynamic weight
                    $itemWeight = $child->getWeight();
                    $itemQty    = $child->getTotalQty();
                    $rowWeight  = $itemWeight*$itemQty;
                    $addressWeight+= $rowWeight;
            ...
            if ($item->getProduct()->getWeightType()) { //fixed weight
                $itemWeight = $item->getWeight();
                $rowWeight  = $itemWeight*$item->getQty();
                $addressWeight+= $rowWeight;
                if ($cartFreeShipping || $item->getFreeShipping()===true) {
        ...
        if ($useDiscountValue && !$useProductPrice){
            if ($useTax && Mage::getStoreConfig('tax/calculation/apply_after_discount', Mage::app()->getStore())) {//PROD-73
                $taxPercentage = $item->getTaxPercent();
                $discountValue = !$useBase ? $item->getDiscountAmount() : $item->getBaseDiscountAmount();
                $discountValue += round($discountValue * ($taxPercentage/100), 2);
