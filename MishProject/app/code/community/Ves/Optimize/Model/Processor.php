...
class Ves_Optimize_Model_Processor
{
    ...
    /**
     * @deprecated after 1.8.0.0 - moved to Ves_Optimize_Model_Container_Viewedproducts
     */
    ...
    protected function _createRequestIds()
    {
        ...
        if ($uri) {
            ...
            if (isset($_COOKIE[Ves_Optimize_Model_Cookie::COOKIE_CUSTOMER_GROUP])) {
                $uri .= '_' . $_COOKIE[Ves_Optimize_Model_Cookie::COOKIE_CUSTOMER_GROUP];
            }
            if (isset($_COOKIE[Ves_Optimize_Model_Cookie::COOKIE_CUSTOMER_LOGGED_IN])) {
                $uri .= '_' . $_COOKIE[Ves_Optimize_Model_Cookie::COOKIE_CUSTOMER_LOGGED_IN];
            }
            if (isset($_COOKIE[Ves_Optimize_Model_Cookie::CUSTOMER_SEGMENT_IDS])) {
                $uri .= '_' . $_COOKIE[Ves_Optimize_Model_Cookie::CUSTOMER_SEGMENT_IDS];
            }
            if (isset($_COOKIE[Ves_Optimize_Model_Cookie::IS_USER_ALLOWED_SAVE_COOKIE])) {
                $uri .= '_' . $_COOKIE[Ves_Optimize_Model_Cookie::IS_USER_ALLOWED_SAVE_COOKIE];
            }
    ...
    protected function _processContainers(&$content)
    {
        $placeholders = array();
        preg_match_all(
            Ves_Optimize_Model_Container_Placeholder::HTML_NAME_PATTERN,
    ...
    public function canProcessRequest(Zend_Controller_Request_Http $request)
    {
        ...
        if ($res) {
            ...
            unset($queryParams[Ves_Optimize_Model_Cache::REQUEST_MESSAGE_GET_PARAM]);
     ...
     *
     * @param Zend_Controller_Request_Http $request
     * @return Ves_Optimize_Model_Processor_Default|false
     */
    ...
    {
        Ves_Optimize_Model_Cache::getCacheInstance()->save(
    ...
    protected function _loadMetadata()
    {
        if ($this->_metaData === null) {
            $cacheMetadata = Ves_Optimize_Model_Cache::getCacheInstance()
