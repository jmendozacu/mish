 ...
 */
class Ves_FAQ_Model_Import_Category extends Mage_Core_Model_Abstract {
			...
			while (($data = fgetcsv($handle, 110000, $this->delimiter)) !== FALSE) {
				...
				foreach ($this->header_columns as $index => $keyname) {
					...
					switch ($keyname) {
						...
						$table = $resource->getTableName('ves_faq/category');
				...
				try {
					$model->save();
					Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('ves_faq')->__('Updated: ' . $name));
				} catch (Exception $e) {
