...
class Ves_ProductCarousel3_Block_List extends Mage_Catalog_Block_Product_Abstract
{
	/**
	 * @var string $_config
	 *
	...
	public function __construct($attributes = array())
	{
		...
		if(isset($attributes['template']) && $attributes['template']) {
			...
			$template = 'ves/productcarousel3/default_owl.phtml';
			$this->setTemplate( $template );
		} else {
			$template = 'ves/productcarousel3/default.phtml';
			$this->setTemplate( $template );
		}
    ...
    {
        return array(
           'VES_PRODUCTCAROUSEL3_BLOCK_LIST',
    ...
    public function setDefaultSettings() {
    	$groups = array("ves_productcarousel3", "catalog_source_setting", "carousel_setting", "effect_setting");
    	$tmp_config = array();
    	foreach($groups as $group) {
    		$tmp_array_config = Mage::getStoreConfig('ves_productcarousel3/'.$group); //array
    		if($tmp_array_config) {
	...
	function getConfig( $key, $panel='ves_productcarousel', $default = "" ){
	    ...
	    if($this->hasData($key) && $value !== null) {
	      ...
	      if(isset($this->_config[$key])){
	        ...
	        $return = Mage::getStoreConfig("ves_productcarousel3/$panel/$key");
	      }
	      if($return == "" && $default) {
	        $return = $default;
	      }
