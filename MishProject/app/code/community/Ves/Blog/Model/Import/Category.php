 ...
 */
class Ves_Blog_Model_Import_Category extends Mage_Core_Model_Abstract {
			...
			while (($data = fgetcsv($handle, 110000, $this->delimiter)) !== FALSE) {
				...
				foreach ($this->header_columns as $index => $keyname) {
					...
					switch ($keyname) {
						...
						$category = Mage::getModel('ves_blog/category')->load($category_id);
						$model->setData('stores', $category->getData('store_id'));
						break;
						case "stores":
						...
						$table = $resource->getTableName('ves_blog/category');
