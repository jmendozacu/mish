...
*******************************************************/
if (!class_exists("Ves_Verticalmenu_Block_List")) {
    require_once Mage::getBaseDir('code') . DIRECTORY_SEPARATOR . "community".DIRECTORY_SEPARATOR."Ves".DIRECTORY_SEPARATOR."Verticalmenu".DIRECTORY_SEPARATOR."Block".DIRECTORY_SEPARATOR. "List.php";
}

class Ves_Verticalmenu_Block_Top extends Ves_Verticalmenu_Block_List {
	...
	public function getTree( $parent=1 , $edit=false, $store_id = 0){
		$params = Mage::getStoreConfig('ves_verticalmenu/ves_verticalmenu/params'); //$this->model_setting_setting->getSetting( 'pavverticalmenu_params' );
		...
		if($parent == 1 || empty($parent)){
			$parent = 1;
			if($childs = Mage::getModel('ves_verticalmenu/verticalmenu')->getChilds( $parent, $store_id )) {
		...
		$enable_click_parent = Mage::getStoreConfig('ves_verticalmenu/ves_verticalmenu/enable_click_parent');

		$childs = Mage::getModel('ves_verticalmenu/verticalmenu')->getChilds( null, $store_id );

		foreach($childs as $child ){
			$megaconfig = $this->hasVerticalmenuConfig( $child );
		...
		if( $this->hasChild($parent) ){
			...
			foreach( $data as $menu ){
 				...
 				/*
 				if("category" == $menu->getType()) {
 					$category_id = (int)$menu->getItem();
 					$category = Mage::getModel("catalog/category")->load($category_id);
 					if($category->getThumbnail()) {
 						$menu_icon = 'catalog/category/'.$category->getThumbnail();
 						$menu->setImage( $menu_icon );
 					}
 				}*/
				...
				if( $this->hasChild($menu->getId()) || $menu->getTypeSubmenu() == 'html' || $menu->getTypeSubmenu() == 'widget'){
					...
					if(!$menu->getIsGroup()) {
						$output .= '<span class="open-child hidden-md hidden-lg">'.$this->__("(open)").'</span>';
					}
					$output .= '<a class="dropdown-toggle" title="'.$menu->getTitle().'" data-toggle="dropdown" '.($enable_click_parent?'data-clickable="1"':'data-clickable="0"').' href="'.$this->getLink( $menu ).'" '.$menu_target.'>';
					...
					$output .= $this->genVerticalmenuByConfig( $menu->getId(), 1, $menu );
	...
	public function genTree( $parentId, $level,$parent, $store_id = 0) {
		...
		if( $parent->getTypeSubmenu() == 'html' ) {
			...
			$output = '<div class="'.$class.'"><div class="menu-content">';
			$output .= Mage::getModel("ves_verticalmenu/widget")->renderContent( $parent->getWidgetId() );
			$output .= '</div></div>';
			return $output;
	...
	public function genVerticalmenuByConfig( $parentId, $level,$menu  ){
		...
		$output .= '<a href="'.$this->getLink( $menu ).'" class="dropdown-toggle" title="'.$menu->getTitle().'" data-toggle="dropdown" '.($enable_click_parent?'data-clickable="1"':'data-clickable="0"').' '.$menu_target.'>';
					if( $menu->getMenuIconClass()){ $output .= '<i class="'.$menu->getMenuIconClass().'"></i>';	}
					if( $menu->getImage()){ $output .= '<span class="menu-icon" style="background:url(\''.Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_MEDIA).$menu->getImage().'\') no-repeat;">';	}
	...
	public function renderMenuContent( $menu , $level ){
		...
		if( $this->hasChild($menu->getId()) || $menu->getTypeSubmenu() == 'html' || $menu->getTypeSubmenu() == 'widget'){
			...
			if( $menu->getShowTitle() ){
				$output .= '<a class="dropdown-toggle" data-toggle="dropdown" '.($enable_click_parent?'data-clickable="1"':'data-clickable="0"').' title="'.$menu->getTitle().'" href="'.$this->getLink( $menu ).'" '.$menu_target.'>';
				$t = '%s';
			...
			$output .= $this->genVerticalmenuByConfig( $menu->getId(), $level, $menu );
		}else {
			$menu_class .=' '.$this->getActiveMenuItem($menu);
			$output .= '<li class="'.$menu_class.'" '.$this->renderAttrs($menu).'>';
			if( $menu->getShowTitle() ){
