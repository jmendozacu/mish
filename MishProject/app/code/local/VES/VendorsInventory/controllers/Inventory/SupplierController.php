 ...
 */
class VES_VendorsInventory_Inventory_SupplierController extends VES_Vendors_Controller_Action {
    /*
     * Menu path
     * 
     * @var string
    ...
    public function gridAction() {
        ...
        $this->getResponse()->setBody(
                $this->getLayout()->createBlock('vendorsinventory/supplier_grid')->toHtml()
        );
    }
    ...
    public function editAction() {
        ...
        if ($model->getId() || $inventorypurchasingId == 0) {
            ...
            Mage::register('vendorsinventory_supplier_data', $model);
            ...
            $this->_addContent($this->getLayout()->createBlock('vendorsinventory/supplier_edit'))
                    ->_addLeft($this->getLayout()->createBlock('vendorsinventory/supplier_edit_tabs'));

            $this->renderLayout();
        } else {
            Mage::getSingleton('vendors/session')->addError(
                    Mage::helper('vendorsinventory')->__('Item does not exist')
            );
            $this->_redirect('*/*/');
        }
    }
    ...
    public function saveAction() {
        if ($data = $this->getRequest()->getPost()) {
            ...
            try {
                ...
                $admin = Mage::getModel('vendors/session')->getUser();
                if (!$this->getRequest()->getParam('id')) {
                    $model->setData('created_by', $admin->getVendorId());
                    $model->setData('vendor_id', $admin->getId());
                }

                Mage::dispatchEvent('ves_vendorsinventory_controller_supplier_save_before', array('model' => $model, 'datas' => $data));
                ...
                if (!$this->getRequest()->getParam('id')) {
                    ...
                    $supplierHistory->setData('supplier_id', $model->getId())
                            ...
                            ->setData('created_by', $admin->getVendorId())
                            ->setData('vendor_id', $admin->getId())
                            ->save();
                    $supplierHistoryContent->setData('supplier_history_id', $supplierHistory->getId())
                            ->setData('field_name', Mage::helper('vendorsinventory')->__('%s created this supplier.', $admin->getVendorId()))
                            ->save();
                } else {
    ...
    public function exportCsvAction() {
        ...
        $content = $this->getLayout()
                ->createBlock('vendorsinventory/supplier_grid')
                ->getCsv();
    ...
    public function productAction() {
        $this->loadLayout();
        $this->getLayout()->getBlock('vendorsinventory.supplier.edit.tab.products')
                ->setProducts($this->getRequest()->getPost('supplier_products', null));
        $this->renderLayout();
        if (Mage::getModel('vendors/session')->getData('supplier_product_import'))
            Mage::getModel('vendors/session')->setData('supplier_product_import', null);
    }
    ...
    public function importproductAction() {
        if (isset($_FILES['fileToUpload']['name']) && $_FILES['fileToUpload']['name'] != '') {
            try {
                ...
                $fields = array();
                $count = 0;
                $supplierHelper = Mage::helper('vendorsinventory/supplier');
                if (count($dataFile))
    ...
    public function showhistoryAction() {
        $form_html = $this->getLayout()
                ->createBlock('vendorsinventory/supplier')
    ...
    public function returnorderAction() {
        $this->loadLayout();
        $this->getLayout()->getBlock('vendorsinventory.supplier.edit.tab.returnorder');
        $this->renderLayout();
    }
    ...
    public function purchaseorderAction() {
        $this->loadLayout();
        $this->getLayout()->getBlock('vendorsinventory.supplier.edit.tab.purchaseorder');
        $this->renderLayout();
    }
