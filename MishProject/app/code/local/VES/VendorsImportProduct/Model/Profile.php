...
class VES_VendorsImportProduct_Model_Profile extends Mage_Dataflow_Model_Profile
{
	/**
     * Run profile
     *
    ...
    public function run()
    {
        ...
        Mage::getModel('dataflow/profile_history')
            ...
            ->setActionCode('run')
            ->setVendorId($this->getVendor()->getId())
            ->save();
...
//            $p['file']['path'] = 'var/export';
            ...
            $fileXml = '<action type="vendorsimport/convert_adapter_io" method="'
                . ($import ? 'load' : 'save') . '">' . $nl;
        ...
        switch ($p['parse']['type']) {
            case 'excel_xml':
                $parseFileXml = '<action type="vendorsimport/convert_parser_xml_excel" method="'
            ...
            case 'csv':
                $parseFileXml = '<action type="vendorsimport/convert_parser_csv" method="'
                    . ($import ? 'parse' : 'unparse') . '">' . $nl;
        ...
        $parsers = array(
            'product'=>'vendorsimport/convert_parser_product',
...
//            $parseDataXml .= '</action>'.$nl.$nl;
        ...
        $adapters = array(
            'product'=>'vendorsimport/convert_adapter_product',
