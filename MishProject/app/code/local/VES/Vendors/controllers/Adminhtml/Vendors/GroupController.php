...
class VES_Vendors_Adminhtml_Vendors_GroupController extends Mage_Adminhtml_Controller_Action
{
	...
	public function saveAction() {
		...
		if ($data = $this->getRequest()->getPost()) {
			...
			try {
				$model->save();
				$this->addGroupcategorybysession($model->getId());
				Mage::getSingleton('core/session')->unsSessCatId();
   ...
   public function addGroupcategoryAction() {
		$data = $this->getRequest()->getPost();
		
		$id = $this->getRequest()->getPost('group_id');
		$cat_id = $this->getRequest()->getPost('selectcategory');
		if($id != ""){
	$group_model = Mage::getModel('vendors/group')->load($id);
		if($cat_id != ""){
		$group_model->setSelectedCategory($cat_id);
	}else{
		$group_model->setSelectedCategory('');
	}
		$group_model->save();
	}else{
		$sess_catid = Mage::getSingleton('core/session')->setSessCatId($cat_id);
	}

		 //Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('vendors')->__(''));
        //$this->_redirect('*/*/');
	echo "Category Updated..!";
	}


	 function addGroupcategorybysession($gid) {
		
		$id = $gid;
		
		$cat_id = Mage::getSingleton('core/session')->getSessCatId();
