...
class FME_Geoipultimatelock_Model_Mysql4_Geoipultimatelock_Collection extends Mage_Core_Model_Mysql4_Collection_Abstract {
    ...
    public function setPriorityOrder($dir = 'ASC') { 
        $this->getSelect()->order("priority {$dir}");
    ...
    public function filterCollection($storeid = null, $rulesid = null, $priority = false, $applyLimit = false, $page = '') {
        ...
        if ($rulesid != null) {
            $this->addIdsFilter($rulesid);
        }
        if ($page != '') {
            $this->addPageFilter($page);
        }
        if ($priority) {
    ...
    public function addCountryFilter($country) {
        $this->getSelect()->where('blocked_countries LIKE "%'.$country.'%"');
        return $this;
    }
    public function addPageFilter($page) {
        $this->getSelect()->orWhere('find_in_set(?, cms_pages)', $page);
        return $this;
    }
}