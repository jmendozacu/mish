...
class Magestore_Inventoryplus_Block_Adminhtml_Adjuststock_Edit extends Mage_Adminhtml_Block_Widget_Form_Container {
    ...
    public function __construct() {
        ...
        $this->_updateButton('reset', 'onclick', 'setLocation(\'' . $this->getUrl('adminhtml/inp_adjuststock/new') . '\')');
        ...
        if ($this->getRequest()->getParam('id')) {
            ...
            if ($model->getStatus() == Magestore_Inventoryplus_Model_Adjuststock::STATUS_PROCESSING) {
                $this->_addButton('resum_confirm', array(
                    'label' => Mage::helper('adminhtml')->__('Resum Confirming'),
        ...
        $this->_formScripts[] = "
            ...
            function confirmAction(){
                ...
                if(validate){
                    if(editForm.validate()) {
                        var r=confirm(\"" . Mage::helper('inventoryplus')->__('Are you sure you want to confirm this stock adjustment? This comfirmation will instantly update product\'s Qty. in this warehouse.') . "\");  
                        if (r==true){
            ...
            function uploadFile() {
                ...
                xhr.open('POST', '" . $this->getUrl('adminhtml/inp_adjuststock/importproduct') . "');
                xhr.send(fd);
            }
