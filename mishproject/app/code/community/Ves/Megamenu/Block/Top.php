...
class Ves_Megamenu_Block_Top extends Ves_Megamenu_Block_List {
	...
	public function getTree( $parent=1 , $edit=false, $store_id = 0){
		...
		$enable_click_parent = Mage::getStoreConfig('ves_megamenu/ves_megamenu/enable_click_parent');
	...
	public function genMegaMenuByConfig( $parentId, $level,$menu  ){
		...
		$output .= '<a href="'.$this->getLink( $menu ).'" class="dropdown-toggle" '.($enable_click_parent?'data-clickable="1"':'data-clickable="0"').' title="'.$menu->getTitle().'" data-toggle="dropdown" '.$menu_target.'>';
