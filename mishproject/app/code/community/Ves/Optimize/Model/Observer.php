 ...
 */
class Ves_Optimize_Model_Observer
{
    /*
    ...
    public function __construct(array $args = array())
    {
        $this->_processor = isset($args['processor'])
            ? $args['processor']
            : Mage::getSingleton('ves_optimize/processor');
        $this->_config = isset($args['config']) ? $args['config'] : Mage::getSingleton('ves_optimize/config');
    ...
    public function processPreDispatch(Varien_Event_Observer $observer)
    {
        ...
        $noCache = $this->_getCookie()->get(Ves_Optimize_Model_Processor::NO_CACHE_COOKIE);
        if ($noCache) {
            Mage::getSingleton('catalog/session')->setParamsMemorizeDisabled(false);
            $this->_getCookie()->renew(Ves_Optimize_Model_Processor::NO_CACHE_COOKIE);
        } elseif ($action) {
    ...
    protected function _saveDesignException()
    {
        ...
        $cacheId = Ves_Optimize_Model_Processor::DESIGN_EXCEPTION_KEY;

        if (!Ves_Optimize_Model_Cache::getCacheInstance()->getFrontend()->test($cacheId)) {
            $exception = Mage::getStoreConfig(self::XML_PATH_DESIGN_EXCEPTION);
            Ves_Optimize_Model_Cache::getCacheInstance()
                ->save($exception, $cacheId, array(Ves_Optimize_Model_Processor::CACHE_TAG));
    ...
    public function validateDataChanges(Varien_Event_Observer $observer)
    {
        ...
        $object = $observer->getEvent()->getObject();
        Mage::getModel('ves_optimize/validator')->checkDataChange($object);
        return $this;
    }
    ...
    public function validateDataDelete(Varien_Event_Observer $observer)
    {
        ...
        $object = $observer->getEvent()->getObject();
        Mage::getModel('ves_optimize/validator')->checkDataDelete($object);
        return $this;
    }
    ...
    public function processEntityAction(Varien_Event_Observer $observer)
    {
        ...
        $object = $observer->getEvent()->getObject();
        Mage::getModel('ves_optimize/validator')->cleanEntityCache($object);
        return $this;
    }
    ...
    {
        Ves_Optimize_Model_Cache::getCacheInstance()->clean(Ves_Optimize_Model_Processor::CACHE_TAG);
        return $this;
    }
    ...
    public function cleanCacheByTags(Varien_Event_Observer $observer)
    {
        ...
        if (empty($tags)) {
            Ves_Optimize_Model_Cache::getCacheInstance()->clean();
            return $this;
        }

        Ves_Optimize_Model_Cache::getCacheInstance()->clean($tags);
        return $this;
    }
    ...
    {
        Ves_Optimize_Model_Cache::getCacheInstance()->getFrontend()->clean(Zend_Cache::CLEANING_MODE_OLD);
        return $this;
    }
    ...
    public function registerCompareListChange(Varien_Event_Observer $observer)
    {
        ...
        $previouseList = $this->_getCookie()->get(Ves_Optimize_Model_Cookie::COOKIE_COMPARE_LIST);
        ...
        sort($ids);
        $this->_getCookie()->set(Ves_Optimize_Model_Cookie::COOKIE_COMPARE_LIST, implode(',', $ids));

        //Recenlty compared products processing
        $recentlyComparedProducts = $this->_getCookie()
            ->get(Ves_Optimize_Model_Cookie::COOKIE_RECENTLY_COMPARED);
        ...
        $this->_getCookie()->set(Ves_Optimize_Model_Cookie::COOKIE_RECENTLY_COMPARED,
    ...
    public function processNewMessage(Varien_Event_Observer $observer)
    {
        ...
        $this->_getCookie()->set(Ves_Optimize_Model_Cookie::COOKIE_MESSAGE, '1');
        return $this;
    }
    ...
    {
        try {
            $productIds = $this->_getCookie()->get(Ves_Optimize_Model_Container_Viewedproducts::COOKIE_NAME);
            if ($productIds) {
    ...
    public function registerWishlistChange(Varien_Event_Observer $observer)
    {
        ...
        $this->_getCookie()->setObscure(Ves_Optimize_Model_Cookie::COOKIE_WISHLIST, $cookieValue);

        // Wishlist items count hash for top link
        $this->_getCookie()->setObscure(Ves_Optimize_Model_Cookie::COOKIE_WISHLIST_ITEMS,
    ...
    public function registerWishlistListChange(Varien_Event_Observer $observer)
    {
        ...
        $blockContainer = Mage::getModel('ves_optimize/container_wishlists');
        Ves_Optimize_Model_Cache::getCacheInstance()->remove($blockContainer->getCacheId());
    ...
    public function registerPollChange(Varien_Event_Observer $observer)
    {
        ...
        $this->_getCookie()->set(Ves_Optimize_Model_Cookie::COOKIE_POLL, $cookieValue);
    ...
    public function registerNewOrder(Varien_Event_Observer $observer)
    {
        ...
        /** @var $blockContainer Ves_Optimize_Model_Container_Orders */
        $blockContainer = Mage::getModel('ves_optimize/container_orders');
        Ves_Optimize_Model_Cache::getCacheInstance()->remove($blockContainer->getCacheId());
        return $this;
    }
    ...
    public function processMessageClearing(Varien_Event_Observer $observer)
    {
        ...
        $this->_getCookie()->delete(Ves_Optimize_Model_Cookie::COOKIE_MESSAGE);
        return $this;
    }
    ...
    public function registerDesignExceptionsChange(Varien_Event_Observer $observer)
    {
        ...
        Ves_Optimize_Model_Cache::getCacheInstance()
            ->save($object->getValue(), Ves_Optimize_Model_Processor::DESIGN_EXCEPTION_KEY,
                array(Ves_Optimize_Model_Processor::CACHE_TAG));
        return $this;
    }
     ...
     *
     * @return Ves_Optimize_Model_Cookie
     */
    protected function _getCookie()
    {
        return Mage::getSingleton('ves_optimize/cookie');
    }
    ...
    {
        $varName = Ves_Optimize_Model_Processor::LAST_PRODUCT_COOKIE;
    ...
    public function updateProductInfo(Varien_Event_Observer $observer)
    {
        ...
        if ($paramsObject instanceof Varien_Object) {
            if (array_key_exists(Ves_Optimize_Model_Cookie::COOKIE_CATEGORY_ID, $_COOKIE)) {
                $paramsObject->setCategoryId($_COOKIE[Ves_Optimize_Model_Cookie::COOKIE_CATEGORY_ID]);
            }
        }
        return $this;
    }
    ...
    public function checkMessages(Varien_Event_Observer $observer)
    {
        ...
        if ($httpHost != $urlHost && Mage::getSingleton('core/session')->getMessages()->count() > 0) {
            $transport->setUrl(Mage::helper('core/url')->addRequestParam(
                $url,
                array(Ves_Optimize_Model_Cache::REQUEST_MESSAGE_GET_PARAM => null)
            ));
        }
        return $this;
    }
    ...
    public function changedCustomerSegmentIds(Varien_Event_Observer $observer)
    {
        ...
        $this->_getCookie()->set(Ves_Optimize_Model_Cookie::CUSTOMER_SEGMENT_IDS, $segmentsIdsString);
    }

    /**
     * Drop top navigation block from cache if category bveses visible/invisible
     *
     * @param Varien_Event_Observer $observer
     */
    public function registerCategorySave(Varien_Event_Observer $observer)
    {
        ...
        if ($category->isObjectNew() ||
            ...
            Ves_Optimize_Model_Cache::getCacheInstance()->clean(Mage_Catalog_Model_Category::CACHE_TAG);
        }
    }
    ...
    public function registerCachedFormKey(Varien_Event_Observer $observer)
    {
        ...
        $cachedFrontFormKey = Ves_Optimize_Model_Cookie::getFormKeyCookieValue();
