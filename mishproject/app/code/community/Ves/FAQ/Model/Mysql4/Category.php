 ...
 */
class Ves_FAQ_Model_Mysql4_Category extends Mage_Core_Model_Mysql4_Abstract
{
    public function _construct()
    {
        $this->_init('ves_faq/category', 'category_id');
    }
	...
	protected function _afterSave(Mage_Core_Model_Abstract $object)
	{
      ...
      // process faq item to store relation
		$this->_getWriteAdapter()->delete($this->getTable('ves_faq/category_store'), $condition);
		...
		if($stores){
			foreach ((array) $object->getData('stores') as $store) {
				...
				$this->_getWriteAdapter()->insert(
					$this->getTable('ves_faq/category_store'), $storeArray
					);
			}
		}else{
   ...
   protected function _afterLoad(Mage_Core_Model_Abstract $object)
   {
      // process faq item to store relation
   	$select = $this->_getReadAdapter()->select()->from(
   		$this->getTable('ves_faq/category_store')
