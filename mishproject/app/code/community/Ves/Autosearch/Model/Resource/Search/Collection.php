 ...
 */
class Ves_Autosearch_Model_Resource_Search_Collection extends Mage_Catalog_Model_Resource_Product_Collection
{
    /**
    ...
    public function addSearchFilter($query)
    {
        ...
        if(count($query_array) > 1) {
            $this->_searchQuery = $query_array;
        }
        if(Mage::helper('catalog/product_flat')->isEnabled()){//Check product flat enable
            $this->_getSearchEntityIdsSqlFlat($query);
            //$this->addFieldToFilter('entity_id', array('in'=>new Zend_Db_Expr($this->_getSearchEntityIdsSqlFlat($query))));
        } else {
    ...
    protected function _checkColumnExists($attribute_code = "") {
        ...
        if($attribute_code) {
            ...
            $table_name = $this->getTable('catalog/product_flat') . '_' . (int)$this->getStoreId();

            $database_name = Mage::getConfig()->getResourceConnectionConfig('default_setup')->dbname;

            $query = "SELECT COLUMN_NAME 
                FROM INFORMATION_SCHEMA.COLUMNS
                WHERE Table_Schema = '".$database_name."' AND TABLE_NAME = '".$table_name."' AND COLUMN_NAME = '".$attribute_code."'";
    ...
    protected function _getSearchEntityIdsSqlFlat($query)
    {
        ...
        foreach ($this->_getAttributesCollection() as $attribute) {
            ...
            if ($this->_isAttributeTextAndSearchable($attribute) && $this->_checkColumnExists($attributeCode)) {
                if(is_array($this->_searchQuery)) {
                    $tmp_where = array();
                    foreach($this->_searchQuery as $key) {
                        $tmp_where[] = $resHelper->getCILike($attributeCode, $key, $likeOptions);
                    }
                    $wheres[] = implode(" AND ", $tmp_where);
                } else {
                    $wheres[] = $resHelper->getCILike($attributeCode, $this->_searchQuery, $likeOptions);
                }
    ...
    {   
        ...
        foreach($options as $key => $value) {
            if($value['attribute_code'] != "status" && $this->_checkColumnExists($value['attribute_code']."_value")) {
                if(is_array($this->_searchQuery)) {
                    $tmp_where = array();
                    foreach($this->_searchQuery as $key) {
                        $tmp_where[] = $resHelper->getCILike($value['attribute_code']."_value", $key, array('position' => 'any'));
                    }
                    $wheres[] = implode(" AND ", $tmp_where);
                } else {
                    $wheres[] = $resHelper->getCILike($value['attribute_code']."_value", $this->_searchQuery, array('position' => 'any'));
                }
            }
        }
        $sql = implode(" OR ", $wheres);
    ...
    protected function _getSearchEntityIdsSql($query)
    {
        ...
        if(is_array($this->_searchQuery)) {
            $tmp_where = array();
            foreach($this->_searchQuery as $key) {
                $tmp_where[] = $resHelper->getCILike($ifValueId, $key, $likeOptions);
            }
            $wheres = implode(" AND ", $tmp_where);
        } else {
            $wheres = $resHelper->getCILike($ifValueId, $this->_searchQuery, $likeOptions);
        }
        /*End Build keyword search query*/
    ...
    protected function _getSearchInOptionSql($query)
    {
        ...
        if(is_array($this->_searchQuery)) {
            $tmp_where = array();
            foreach($this->_searchQuery as $key) {
                $tmp_where[] = $resHelper->getCILike($ifValue, $key, array('position' => 'any'));
            }
            $wheres = implode(" AND ", $tmp_where);
        } else {
            $wheres = $resHelper->getCILike($ifValue, $this->_searchQuery, array('position' => 'any'));
        }
        /*End build keyword search*/
