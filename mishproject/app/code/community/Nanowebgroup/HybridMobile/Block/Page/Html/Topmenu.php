 ...
 */
class Nanowebgroup_HybridMobile_Block_Page_Html_Topmenu extends Mage_Core_Block_Template
{
    /**
     * Top menu data tree
     *
    ...
    {
        $html = '';
        $configValue = Mage::getStoreConfig('hybrid_mobile/menu_settings/cat_img', Mage::app()->getStore());
        ...
        foreach ($children as $child) {
            ...
            if($parentLevel == NULL && $childLevel == 0 && $configValue) {
                preg_match_all('!\d+!', $child->getId(), $id);
                $category = Mage::getModel('catalog/category')->load($id);
                if ($image = $category->getThumbnail()) {
                    $imageUrl = Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_MEDIA).'catalog/category/'.$image;
                } else {
                    $imageUrl = "http://placehold.it/100x64";
                }
                $html .= '<a href="' . $child->getUrl() . '" class="menu-item with-image">';
                $html .= '<div class="menu-item-image" style="background-image:url('.$imageUrl.')"></div>';
                ...
                $html .= '<a href="' . $child->getUrl() . '" class="menu-item">';
                if ($child->hasChildren()) {
                    $html .= ' <i class="fa fa-angle-right"></i>';
                }
                $html .= $this->escapeHtml($child->getName());
            ...
            if ($child->hasChildren()) {
                ...
                $html .= '<div class="menu-layer">';
                $html .= '<div class="swiper-container">';
                $html .= '<div class="swiper-wrapper">';
                    $html .= '<div class="swiper-slide">';
                    $html .= '<a class="menu-item" href="'.$child->getUrl().'">';
                    $html .= $this->escapeHtml("All ".$child->getName());
                    $html .= '</a>';
                    $html .= '</div>';
