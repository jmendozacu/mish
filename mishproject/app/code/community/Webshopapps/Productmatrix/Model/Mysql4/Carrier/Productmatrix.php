...
class Webshopapps_Productmatrix_Model_Mysql4_Carrier_Productmatrix extends Mage_Core_Model_Mysql4_Abstract
{
    ...
    public function getNewRate(Mage_Shipping_Model_Rate_Request $request, &$errorText)
    {
        ...
        if ($this->_debug) {
            Mage::helper('wsalogger/log')->postDebug('productmatrix', 'Result Set before Manipulation', $absoluteResults);
        }

        $this->manipulatePricing($absoluteResults, $conditionName);

        if ($this->_debug) {
            Mage::helper('wsalogger/log')->postInfo('productmatrix', 'Result Set ater Manipulation', $absoluteResults);
        }
    ...
    protected function initGlobals()
    {
        ...
        $this->_aTotallingRequired = false;
    ...
    protected function manipulatePricing(&$absoluteResults, $conditionName)
    {
        ...
        // look at percentage algorithms applying on sets of groups. Currently only works on per package
        if ($this->_aTotallingRequired) {
            foreach ($absoluteResults as $key => $data) {
                if ($data['a_total']!=0) {
                    $percPrice = ($data['a_total'] * $data['a_percentage'] / 100);
                    if ($percPrice > $data['a_min_price']) {
                        $absoluteResults[$key]['price'] += $percPrice;
                    } else {
                        $absoluteResults[$key]['price'] += $data['a_min_price'];
                    }
                }
            }
        }
            ...
            foreach ($indResults as $indKey => $data) {
                if ($result['delivery_type'] == $data['delivery_type']) {
                    ...
                    if ($baseResults[$key]['override']) {
                        ...
                        $baseResults[$key]['priority']=$data['priority'];//MD-130
                        $found = true;
                        break;
                    } else {
                        // add to existing
                        $baseResults[$key]['price'] += $data['price'];
                        $baseResults[$key]['a_total'] += $data['a_total'];
                        if ($data['a_min_price']>0) {
                            $baseResults[$key]['a_min_price'] = $data['a_min_price'];
                        }
                        if ($data['a_percentage']>0) {
                            $baseResults[$key]['a_percentage'] = $data['a_percentage'];
                        }
                        $baseResults[$key]['package_id'] = $baseResults[$key]['package_id'] . "," . $data['package_id'];
                        $found = true;
                        break;
                ...
                if (!empty($algorithm) && count($algorithm) == 2) {
                    ...
                    switch ($algKey) {
                        ...
                        case "o":
                            ...
                            break;
                        case "min_a":
                            $data['a_min_price']=$algValue;
                            break;
                        case "%a":
                            $this->_aTotallingRequired = true;
                            $data['a_total'] += $structuredItem['price'];
                            $data['a_percentage'] = $algValue;
                            break;
                        case "%":
                        ...
                        case "setcart":
                            ...
                            break;
                        case "cutoff":
                            if ($this->isLaterThanCutOffTime($algValue)) {
                                $skipData = true;
                            }
                            break;
                        case "day":
                            if (!$this->isApplicableDay($algValue)) {
                                $skipData = true;
                            }
                            break;
                        case "instock":
                        ...
                        case "coupon": //PROD-47
                            if (Mage::app()->getStore()->isAdmin()) {
                                $cartCouponCode = Mage::getSingleton('adminhtml/session_quote')->getQuote()->getCouponCode();
                            } else {
     ...
     * @return bool
     */
    protected function isLaterThanCutOffTime($cutoffTime)
    {
        try {
            if ($cutoffTime != "" && (!strtotime("January 1 1970 " . $cutoffTime) === false)) {
                ...
                if ($this->_debug) {
                    Mage::helper('wsalogger/log')->postDebug('productmatrix', 'Cuffoff Logic', 'Found current system time as: ' . $currentTime);
                    Mage::helper('wsalogger/log')->postDebug('productmatrix', 'Cuffoff Logic', 'Cutoff time is: ' . $cutoffTime);
                }
    ...
    /**
     * Checks to see if the defined available day/s is today.
     * PROD-64
     *
     * @param int $day
     * @return bool
     */
    protected function isApplicableDay($days)
    {
        try {
            $dayArr = explode(",", $days);
            ...
            if ($this->_debug) {
                Mage::helper('wsalogger/log')->postDebug('productmatrix', 'Available Day Logic', 'Available Days: ' . $dayArr);
                Mage::helper('wsalogger/log')->postDebug('productmatrix', 'Available Day Logic', 'Found current day as: ' . $today);
            }

            if (in_array($today, $dayArr)) {
                return true;
            }
        } catch (Exception $e) {
            Mage::log($e->getMessage());
            Mage::helper('wsalogger/log')->postCritical('productmatrix', 'Date Error', 'Error Processing Day of Week logic. See Exception log.');
        }
    ...
    private function populateStructuredItems($items, $conditionName)
    {
        ...
        foreach ($items as $item) {
            ...
            if ($product->getAttributeText('package_id') == 'SKU' && in_array('use_sku', $this->_options)) { //PROD-68
                if ($this->_debug) {
                    Mage::helper('wsalogger/log')->postDebug('productmatrix',
                                                             'Custom Option SKU used as shipping group for item ID',
                                                             $item->getId()
                                                             );
                }

                $packageIdArray[] = $item->getProduct()->getData('sku');//Add basic SKU to package ID array.

                $options = Mage::helper('catalog/product_configuration')->getCustomOptions($item);

                foreach ($options as $option) {
                    $option = Mage::getModel('catalog/product_option_value')->load($option['option_id']);

                    $packageIdArray[] = $option->getSku(); //Add each option SKU to package ID array.
                }
            } elseif($attribute == 'warehouse' && (Mage::helper('wsacommon')->isModuleEnabled('Webshopapps_Dropship') ||
                                             Mage::helper('wsacommon')->isModuleEnabled('Webshopapps_Shipmanager'))) {
                ...
                if ($this->_debug) {
                    Mage::helper('wsalogger/log')->postDebug('productmatrix',
                                                             'Using warehouse as shipping group',
                                                             ''
                ...
                $packageIdArray[] = Mage::helper('dropcommon/shipcalculate')
                    ->determineWhichWarehouse($item,
                ...
                if (in_array('group_text', $this->_options)) {
                    $packageIdArray[] = $product->getData($attribute);
                } else {
                    $packageIdArray[] = $product->getAttributeText($attribute);
                }
            }

            if (empty($packageId) && is_numeric($freightQtyThreshold) && $qty >= $freightQtyThreshold) {
                $packageIdArray[] = 'SHIP_FREIGHT_AUTO';
            }
            ...
            foreach ($packageIdArray as $packageId) {
                ...
                if ($splitCustom && !in_array('use_sku', $this->_options)) {
                    $options = Mage::helper('catalog/product_configuration')->getCustomOptions($item);
                                        ...
                                        foreach ($splitCountries as $country) {
                                            ...
                                            foreach ($splitRegions as $region) {
                                                ...
                                                if (!isset($countryCodesToIds[$country])
                                                    ...
                                                    // need to do because of bug with AE state code, may have multiple regions with the same id
