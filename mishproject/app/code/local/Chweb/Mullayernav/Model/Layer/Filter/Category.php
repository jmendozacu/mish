...
* @Class        Chweb_Mullayernav_Model_Layer_Filter_Category   
*/
class Chweb_Mullayernav_Model_Layer_Filter_Category extends Mage_Catalog_Model_Layer_Filter_Category {

    protected $cat = null;

    public function apply(Zend_Controller_Request_Abstract $request, $filterBlock) {
        // very small optimization
        $catId = (int) Mage::helper('mullayernav')->getParam($this->getRequestVar());
        if ($catId) {
    ...
    protected function _getItemsData() {
        ...
        $key .= Mage::helper('mullayernav')->getCacheKey('cat');
        $pageKey = Mage::getBlockSingleton('page/html_pager')->getPageVarName();
        $queryStr = Mage::helper('mullayernav')->getParams(true, $pageKey);
        $data = $this->getLayer()->getAggregator()->getCacheData($key);

        if ($data === null) {
            $category = null;
            $showTopCategories = Mage::getStoreConfig('mullayernav/mullayernav/top_cats');
            ...
            if (Mage::getStoreConfig('mullayernav/mullayernav/reset_filters')) {
                $queryStr = '';
            }
