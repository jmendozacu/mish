...
class Mirasvit_Seo_Model_Rewrite_Product_Image extends Mage_Catalog_Model_Product_Image
{
	public function setBaseFile($file)
    {
        ...
        $newPath[] = 'product';
        $newPath[] = substr(md5('/catalog/product'.$file.Mage::app()->getStore()->getId()."{$this->_width}x{$this->_height}"), 0, 2);
        $md5 = md5($this->_baseFile);
        $newPath[] = $this->getUrlKey().'-'.substr($md5, 3, 2).'.'.$path_info['extension'];
