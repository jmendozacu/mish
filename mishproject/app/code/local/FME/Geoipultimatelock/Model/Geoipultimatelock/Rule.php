 ...
 */
class FME_Geoipultimatelock_Model_Geoipultimatelock_Rule extends Mage_CatalogRule_Model_Rule {
    ...
    protected $_retMode = 2; // if no rule, then all products will be choosen
    ...
    public function getMatchingProductIds() {
        ...
        if ($sql) {
            try {
                ...
                $this->_matchedCollection->getSelect()->order('e.entity_id ASC'); //echo (string)$this->_matchedCollection->getSelect();exit;
                return $this->_matchedCollection->getAllIds();
            } catch (Exception $ex) {
                Mage::log('Exception: ' . $ex . ' in ' . __CLASS__ . ' on ' . __LINE__, null, 'FME_Geoipultimatelock.log');
            }
        } else
            return null;
    }
    ...
    protected function addRulesToMatch($rules, $aggregator, $condition) {
        ...
        foreach ($rules as $rule) {//echo $rule->getAttribute();exit;
    ...
    public function prepareSqlForAtt($attribute, $joinedAttributes, $collection, $operator, $value) {
        ...
        $operator = ($attribute == 'category_ids' OR $attribute == 'sku') ? str_replace($cnOperator, $cmOperator, $operator) : str_replace($nOperator, $mOperator, $operator); //echo $operator;
            ...
            switch ($attribute) {
                case 'sku':
                    $collection->getSelect()
                            ->joinRight(array('att_table_' . $attribute => $att->getBackend()->getTable()), 'att_table_' . $attribute . '.entity_id = e.entity_id', array('sku')); //echo (string) $collection->getSelect();
                    break;
                case 'category_ids':
                    $collection->getSelect()
                ...
                case 'attribute_set_id':
                    ...
                    break;
                case 'price':
                    if (Mage::helper('productlabelspro')->checkVersion('1.4.1.1')) {
                        $collection->getSelect()
        ...
        switch ($attribute) {
            case 'sku':
                $where = '(att_table_' . $attribute . '.' . 'sku' . ' ' . $operator . ' ' . $value . ')'; // echo $where;  
                break;
            case 'category_ids':
    ...
    protected function _impValues($str) {
        $newStr = explode(',', $str);
