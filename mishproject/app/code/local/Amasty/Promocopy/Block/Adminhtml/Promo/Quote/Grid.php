...
class Amasty_Promocopy_Block_Adminhtml_Promo_Quote_Grid extends Mage_Adminhtml_Block_Promo_Quote_Grid
{
    ...
    $model = Mage::getModel('salesrule/rule')->getCollection()->getData();
       
        $vendorids=array();
      
        foreach ($model as $models) {
           $vendorids[]=$models['vendor_id'];
            }
  ...
  if ($vendorid=='') {
        ...
        $this->addColumnAfter('discount_amount', array(
         ...
         $this->addColumnAfter('vendor_id', array(
            'header'    => Mage::helper('sales')->__('Vendor_id'), //its correct
            'align'     => 'right',
            'index'     => 'vendor_id',
        ...
        $this->addColumn('action',array(
            ...
            'actions'   => array(
                array(
                    ...
                    'url'     => array('base' => 'vendors/promocopy_index/duplicate'),
                    'field'   => 'rule_id'
                )
            ),
            'filter'    => false,
    ...
    protected function addAction($label, $urlKey='', $isConfim=false)   
    {
        $this->getMassactionBlock()->addItem(str_replace('/', '_', $urlKey), array(
             'label'    => $this->__($label),
             'url'      => $this->getUrl('vendors/promocopy_index/'. $urlKey),
             // 'url'      => $this->getUrl('vendors/import_ampromocopy/index/'. $urlKey),
