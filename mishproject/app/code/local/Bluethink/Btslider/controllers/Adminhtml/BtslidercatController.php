...
class Bluethink_Btslider_Adminhtml_BtslidercatController extends Mage_Adminhtml_Controller_action
{
	...
	public function savecatBanerimagesAction() {
		$dataa= $this->getRequest()->getPost();
        if(isset($_FILES['fileToUploadcat']['name']) && $_FILES['fileToUploadcat']['name'] != '')
            {
               $fname1 = $_FILES['fileToUploadcat']['name'] ;
         try
           {       
	        $path = Mage::getBaseDir('media').DS."slider".DS;  //desitnation directory     
	        $fname =$profileimagecat; //file name
	        $uploader = new Varien_File_Uploader('fileToUploadcat'); //load class
	        ...
	        $profileimagecat = $uploader->getUploadedFileName();
    ...
    catch (Exception $e)
    {
        echo 'Error Message: '.$e->getMessage();
         ...
         $title = $this->getRequest()->getPost('titlecat');
         $url=  $this->getRequest()->getPost('urlcat');
       

	        $model = Mage::getModel('btslider/btcategoryslider')->load();
	    
	        $model->setCategoryid($categoryid);
	        $model->setSliderimage($profileimagecat);
	        $model->setUrl($url);
  ...
  public function deletecatsliderimageAction()
    {
 
            $catsliderid = Mage::app()->getRequest()->getParam('id');

            $delData = Mage::getModel('btslider/btcategoryslider')->load($catsliderid);
