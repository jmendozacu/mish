...
class VES_Vendors_Model_Vendor extends Mage_Core_Model_Abstract {
    ...
    const XML_PATH_REJECT_EMAIL_TEMPLATE = 'vendors/create_account/email_template_rejected';
    const XML_PATH_FINISH_EMAIL_TEMPLATE = 'vendors/create_account/email_template_finish';
    ...
    public function sendNewAccountEmail($type = 'registered', $backUrl = '', $storeId = '0') {
        $types = array(
            ...
            'reject' => self::XML_PATH_REJECT_EMAIL_TEMPLATE, // Reject email, when Vendor is reject
            'finish' => self::XML_PATH_FINISH_EMAIL_TEMPLATE, // Reject email, when Vendor configuration is finish
        );
        if (!isset($types[$type])) {
        ...
        $this->_sendEmailTemplate($types[$type], self::XML_PATH_REGISTER_EMAIL_IDENTITY, array('vendor' => $this, 'back_url' => $backUrl), $storeId);
        return $this;
    }
    ...
    public function sendPasswordResetConfirmationEmail() {
        ...
        $this->_sendEmailTemplate(self::XML_PATH_FORGOT_EMAIL_TEMPLATE, self::XML_PATH_FORGOT_EMAIL_IDENTITY, array('vendor' => $this), $storeId);
    ...
    public function authenticate($login, $password) {
        ...
        if (!$this->validatePassword($password)) {
            throw Mage::exception('Mage_Core', Mage::helper('vendors')->__('Invalid login or password.'), self::EXCEPTION_INVALID_EMAIL_OR_PASSWORD
            );
        }

        if ($this->getConfirmation() && $this->isConfirmationRequired()) {
            throw Mage::exception('Mage_Core', Mage::helper('vendors')->__('Your vendor account is not confirmed.'), self::EXCEPTION_EMAIL_NOT_CONFIRMED
            );
        }

        if ($this->getStatus() == self::STATUS_PENDING) {
            throw Mage::exception('Mage_Core', Mage::helper('vendors')->__('Your vendor account is awaiting for approval.'), self::EXCEPTION_ACCOUNT_SUPPENDED
            );
        }

        if ($this->getStatus() == self::STATUS_DISABLED) {
            throw Mage::exception('Mage_Core', Mage::helper('vendors')->__('Your vendor account has been suppended.'), self::EXCEPTION_ACCOUNT_SUPPENDED
            );
        }
    ...
    /**
     * Load vendor by vendor_token
     *
     * @param   string $vendorToken
     * @return  VES_Vendor_Model_Vendor
     */
    public function loadByToken($vendorToken, $websiteId = false) { 
        if (!$websiteId)
            $websiteId = Mage::app()->getWebsite()->getId();
        if ($websiteId)
            $this->setWebsiteId($websiteId);
        $this->_getResource()->loadByToken($this, $vendorToken);
        return $this;
    }
