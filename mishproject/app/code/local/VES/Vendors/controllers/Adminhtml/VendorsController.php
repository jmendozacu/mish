	...
	public function saveAction() {
		if ($data = $this->getRequest()->getPost()) {
			...
			try {
				...
				// Activation mail
				if($model->getStatus() == VES_Vendors_Model_Vendor::STATUS_ACTIVATED && !$model->getData("is_sendmail_active_vendor")){
						if(Mage::getStoreConfig('vendors/create_account/send_approved')){
							$model->sendNewAccountEmail("active");
							$model->setData("is_sendmail_active_vendor",1);
						}
					}
					else{
						$model->setData("is_sendmail_active_vendor",0);
					}

					// Rejection mail
				if($model->getStatus() == VES_Vendors_Model_Vendor::STATUS_REJECTED && !$model->getData("is_sendmail_active_vendor")){
						if(Mage::getStoreConfig('vendors/create_account/send_approved')){
							$model->sendNewAccountEmail("reject");
							$model->setData("is_sendmail_active_vendor",1);
							$model->save();
						}
					}
