<?php
class VES_VendorsCommissionCalculation_Model_Credit_Type_Itemcommission extends VES_VendorsCredit_Model_Type_Abstract
{
	public function process(){
    	...
    	$description		= Mage::helper('vendorscommission')->__('Commission of order #%s, item %s x %s',$order->getIncrementId(),$invoiceItem->getName(),$invoiceItem->getQty()*1);
	...
	public function getDescription(VES_VendorsCredit_Model_Transaction $transaction){
		...
		$description	= Mage::helper('vendorscommission')->__('Commission of order #%s, item %s x %s',$order->getIncrementId(),$orderItem->getName(),$orderItem->getQtyOrdered()*1);
		return $description;
	}
}