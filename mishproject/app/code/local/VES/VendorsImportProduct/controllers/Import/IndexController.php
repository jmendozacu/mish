<?php
class VES_VendorsImportProduct_Import_IndexController extends VES_Vendors_Controller_Action
{   
	...
	protected function _initProfile()
    {
        ...
        $profileId = (int) Mage::helper('vendorsimport')->getImportProfile($fileExtension);
    ...
    public function deleteAction(){
    	...
    	foreach($files as $file){
    		@unlink(Mage::helper('vendorsimport')->getVendorImportFile($file, $vendor));
    	}
	        ...
	        while (false !== ($entry = readdir($handle))) {
		        if ($entry != "." && $entry != ".." && is_file($dir.$entry)) {
		            $images[] = array(
		            	...
		            	'thumbnail_url'	=> Mage::getDesign()->getSkinUrl('ves_vendors/importproduct/icons/file.png'),
		            	'file_name'		=> $entry,
