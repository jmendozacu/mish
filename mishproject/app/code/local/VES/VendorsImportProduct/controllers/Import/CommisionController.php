<?php
class VES_VendorsImportProduct_Import_CommisionController extends VES_Vendors_Controller_Action
{   
	...
	protected function _initAction() {
		$this->loadLayout()
			->_setActiveMenu('commission')
			->_title($this->__('Module'))
			->_title($this->__('Commission'))
			->_addBreadcrumb($this->__('commission'), $this->__('Module'));
    ...
    $vendorid = Mage::app()->getRequest()->getPost('vendorid');
    
    

      $catData= $data[parentcategory];
      $implodeData = implode(",",$catData);



  $vendormodeladd = Mage::getModel('vendors/additional')->load($vendorid,'vendor_id');
     //$vendormodeladd = Mage::getModel('vendors/additional')->getCollection()
      //->addFieldToFilter('vendor_id',array('eq'=>23));

   
      if($implodeData != ""){
      $vendormodeladd->setCategories($implodeData);
  }else{
        $vendormodeladd->setCategories("");
  }
      $vendormodeladd->save();
       $this->_redirectReferer();
    }

      public function managecommisionAction(){
