...
class VES_VendorsImportProduct_Vendors_RuleController extends VES_Vendors_Controller_Action
{
    ...
    public function indexAction ()
    {
        ...
        $this->_addContent($this->getLayout()
            ->createBlock('helpdesk/vendors_rule'));
        $this->renderLayout();
    }
    ...
    public function addAction ()
    {
        ...
        $this->_addBreadcrumb(Mage::helper('adminhtml')->__('Rule  Manager'),
                Mage::helper('adminhtml')->__('Rule Manager'), mage::getUrl('*/*/'));
        $this->_addBreadcrumb(Mage::helper('adminhtml')->__('Add Rule '), Mage::helper('adminhtml')->__('Add Rule'));

        $this->getLayout()
            ->getBlock('head')
            ->setCanLoadExtJs(true);
        $this->_addContent($this->getLayout()->createBlock('helpdesk/vendors_rule_edit'))
                ->_addLeft($this->getLayout()->createBlock('helpdesk/vendors_rule_edit_tabs'));
    ...
    public function editAction ()
    {
        ...
        if ($rule->getId()) {
            ...
            $this->_addBreadcrumb(Mage::helper('adminhtml')->__('Workflow Rules'),
                    Mage::helper('adminhtml')->__('Workflow Rules'), mage::getUrl('*/*/'));
            ...
            Mage::getSingleton('vendors/session')->addError(Mage::helper('adminhtml')->__('The Rule does not exist.'));
            $this->_redirect('*/*/');
        }
    }
    ...
    public function saveAction ()
    {
        if ($data = $this->getRequest()->getPost()) {
            ...
            try {
                ...
                Mage::getSingleton('vendors/session')->addSuccess(Mage::helper('adminhtml')->__('Rule was successfully saved'));
        ...
        Mage::getSingleton('vendors/session')->addError(Mage::helper('adminhtml')->__('Unable to find Rule to save'));
        $this->_redirect('*/*/');
    }
    ...
    {
        $idss = $this->getRequest()->getParam('rule_id');
         $ids=explode(',', $idss);
        if (!is_array($ids)) {
            Mage::getSingleton('vendors/session')->addError(Mage::helper('adminhtml')->__('Please select Rule(s)'));
        } else {
            try {
