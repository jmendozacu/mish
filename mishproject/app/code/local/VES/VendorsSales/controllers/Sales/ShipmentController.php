...
class VES_VendorsSales_Sales_ShipmentController extends VES_Vendors_Controller_Action
{
    ...
    public function pdfshipmentsAction(){
        ...
        if (!empty($shipmentIds)) {
            $shipments = Mage::getResourceModel('sales/order_shipment_collection')
                ...
                ->load();
                // echo "bhasdjd<pre>";
                // print_r( $shipments->getData());
                // exit();
            if (!isset($pdf)){
                $pdf = Mage::getModel('invoice123/order_pdf_shipment')->getPdf($shipments);
            } else {
                $pages = Mage::getModel('invoice123/order_pdf_shipment')->getPdf($shipments);
    ...
    public function manifestodocumentAction(){
        $shipmentIds = $this->getRequest()->getPost('shipment_ids');
        $shipmentIds = explode(",", $shipmentIds);
        if (!empty($shipmentIds)) {
            $shipments = Mage::getResourceModel('sales/order_shipment_collection')
                ...
                ->load();
            if (!isset($pdf)){
                $pdf = Mage::getModel('deliverydate/order_pdf_shipment')->getPdf($shipments);
            } else {
                $pages = Mage::getModel('deliverydate/order_pdf_shipment')->getPdf($shipments);
                $pdf->pages = array_merge ($pdf->pages, $pages->pages);
            }

            return $this->_prepareDownloadResponse('manifestodocument'.Mage::getSingleton('core/date')->date('Y-m-d_H-i-s').'.pdf', $pdf->render(), 'application/pdf');
        }
        $this->_redirect('*/*/');
    }
}