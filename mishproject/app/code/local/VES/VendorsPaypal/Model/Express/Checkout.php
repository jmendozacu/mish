<?php
class VES_VendorsPaypal_Model_Express_Checkout extends Mage_Paypal_Model_Express_Checkout
{
	/**
    ...
    public function place($token, $shippingMethodCode = null)
    {
        ...
        if(!Mage::helper('vendors')->moduleEnabled()) return parent::place($token,$shippingMethodCode);
        
        
        /*Return if it's not advanced mode*/
        if(Mage::helper('vendors')->getMode() != VES_Vendors_Model_Vendor::MODE_ADVANCED_X) return parent::place($token,$shippingMethodCode);
        ...
        if((sizeof($shippableItems) + sizeof($notShippableItems)) >=1){
            ...
            */
            ...
            Mage::getSingleton('checkout/session')->setLastQuoteId($this->_quote->getId())
            ...
            */
            foreach($shippableItems as $key=>$items){
                ...
                $this->_quote->addAddress($sAddress);
            ...
            try {
                ...
                $redirectUrl = $this->_quote->getPayment()->getOrderPlaceRedirectUrl();
    
                Mage::getSingleton('checkout/session')->setOrderIds($orderIds)
                ...
                Mage::dispatchEvent('checkout_submit_all_after', array('orders' => $orders, 'quote' => $this->_quote));
    ...
    public function copyItemToQuote(Mage_Sales_Model_Quote_Address $address, Mage_Sales_Model_Quote $quote){
        ...
        foreach($address->getAllItems() as $item){
            ...
            $quoteItem = $this->_quote->getItemById($quoteItemId);
    ...
    public function getTmpQuote($vendorId,Mage_Sales_Model_Quote_Address $billingAddress, Mage_Sales_Model_Quote_Address $shippingAddress){
        ...
        $payment = clone $this->_quote->getPayment();
    ...
    public function getShippingAddressByVendorId($vendorId){
        foreach ($this->_quote->getAllShippingAddresses() as $address) {
