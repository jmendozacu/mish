...
class VES_VendorsRma_Vendor_Rma_RequestController extends VES_Vendors_Controller_Action
{
    ...
    public function saveNoteAttacmentAction(){
        ...
        if($request->getId() && $note->getId() ){
            //$newAttachments = Mage::getModel("vendorsrma/source_file")->getActachment($post["filename"]);
            ...
            if(count($newFiles)) {
                ...
                Mage::getSingleton('vendors/session')->addError(Mage::helper('vendorsrma')->__("Do not have any file upload"));
                $this->_redirect('*/*/edit',array('id'=>$id));
                return;
            }
            try {
                $note->save();
                Mage::getSingleton('vendors/session')->addSuccess(Mage::helper('vendorsrma')->__('Upload successfully'));
                Mage::getSingleton('vendors/session')->setFormData(false);
            } catch (Exception $e) {
                Mage::getSingleton('vendors/session')->addError(Mage::helper('vendorsrma')->__($e->getMessage()));
            }
            $this->_redirect('*/*/edit',array('id'=>$id));
            return;
        }
        Mage::getSingleton('vendors/session')->addError(Mage::helper('vendorsrma')->__("Request or Note do not exist"));
         $this->_redirect('*/*/edit',array('id'=>$id));
        return;
    }
    ...
    public function saveNoteAction(){
        ...
        $post = $this->getRequest()->getPost("note");
        if($request->getId()){
            //$attachments = Mage::getModel("vendorsrma/source_file")->getActachmentNote($post["filenamenote"]);
            $files = Mage::getModel("vendorsrma/source_file")->uploadFileNoteVendor($post["filenamenote"]);
            if(!$post["message"]) {
            ...
            try {
                ...
                Mage::getSingleton('vendors/session')->addSuccess(Mage::helper('vendorsrma')->__('Rma was successfully send note'));
                Mage::getSingleton('vendors/session')->setFormData(false);
        ...
        Mage::getSingleton('vendors/session')->addError(Mage::helper('vendorsrma')->__("Request do not exist"));
    ...
    public function editAction() {
        ...
        if ($model->getId() || $request_id == 0) {
            ...
            if($request->getState() == VES_VendorsRma_Model_Option_State::STATE_AWAITING && $request->getFlagState() == 2 ){
            ...
            $this->_addBreadcrumb(Mage::helper('adminhtml')->__('RMA View'), Mage::helper('adminhtml')->__('RMA View'));

            $this->getLayout()->getBlock('head')->setCanLoadExtJs(true);

            $this->_addContent($this->getLayout()->createBlock('vendorsrma/vendor_request_edit')->setTemplate('ves_vendorsrma/form/container.phtml'));
    ...
    public function newAction() {
        ...
        Mage::dispatchEvent('vendor_rma_new_action', array('request' => $request));
        $this->loadLayout();
        $this->renderLayout();
    ...
    public function approvalAction(){
        ...
        try {
            $options = Mage::getModel("vendorsrma/status")->getOptions();
            $request->setUpdateAt($time);
            $request->setStatus($options[1]["value"]);
            $request->save();
            $request->saveNotify();
            $request->saveStatusHistory(VES_VendorsRma_Model_Option_Status_Type::TYPE_VENDOR,$vendor->getTitle());
            Mage::getSingleton('vendors/session')->addSuccess(Mage::helper('vendorsrma')->__('Rma was successfully approval'));
            Mage::getSingleton('vendors/session')->setFormData(false);
    ...
    public function confirmshipAction(){
        ...
        try {
            $options = Mage::getModel("vendorsrma/status")->getOptions();
            $request->setUpdateAt($time);
            $request->setStatus($options[6]["value"]);
            $request->save();
            $request->saveNotify();
            $request->saveStatusHistory(VES_VendorsRma_Model_Option_Status_Type::TYPE_VENDOR,$vendor->getTitle());
            Mage::getSingleton('vendors/session')->addSuccess(Mage::helper('vendorsrma')->__('Rma was successfully confirm shipping'));
            Mage::getSingleton('vendors/session')->setFormData(false);
    ...
    public function escalateAction() {
        ...
        if ($model->getId() || $id == 0) {
            ...
            $this->_addContent($this->getLayout()->createBlock('vendorsrma/vendor_request_escalate'));
    
            $this->renderLayout();
        } else {
            Mage::getSingleton('vendors/session')->addError(Mage::helper('vendorsrma')->__('Request does not exist'));
            $this->_redirect('*/*/');
        }
    }
    ...
    public function markAction(){
        ...
        try {
            ...
            $request->setStatus($options[5]["value"]);
            $request->setState(VES_VendorsRma_Model_Option_State::STATE_BEING);
            $request->setFlagState(6);
            $request->save();
            $request->saveNotify();
            $request->saveStatusHistory(VES_VendorsRma_Model_Option_Status_Type::TYPE_VENDOR,$vendor->getTitle());
            
            $sender = array("from"=>Mage::helper("vendorsrma/config")->getContactNameVendor($vendor),"to"=>$request->getCustomerName());
            $message=array(
                'message'=> $vendor->getVendorId()." ".$this->__("accepted the refund request."),
                'attachment'=> null,
                'created_time'=> now(),
                'attachment'=> "",
                'type'=> VES_VendorsRma_Model_Source_Message_Type::TYPE_VENDOR,
            ...
            Mage::getSingleton('vendors/session')->addSuccess(Mage::helper('vendorsrma')->__('Rma was successfully resolved'));
            Mage::getSingleton('vendors/session')->setFormData(false);
    ...
    public function saveAction() {
        ...
        if ($data = $this->getRequest()->getPost()) {
            ...
            if($validate_qty != VES_VendorsRma_Helper_Data::QTY_APECPT){
                ...
                Mage::getSingleton('vendors/session')->addError($mesage);
                Mage::getSingleton('vendors/session')->setRequestData($data);
            ...
            try {
                ...
                if(!$isEdit) $request->saveNotify();

                /* request save Message */
                $vendor = $this->_getSession()->getVendor();
                $sender = array("from"=>Mage::helper("vendorsrma/config")->getContactNameVendor($vendor),"to"=>$request->getCustomerName());

                $request->saveMessage($this->getRequest()->getPost("request"),VES_VendorsRma_Model_Source_Message_Type::TYPE_VENDOR,$sender,$isEdit);


                Mage::getSingleton('vendors/session')->addSuccess(Mage::helper('vendorsrma')->__('Request was successfully saved'));
                Mage::getSingleton('vendors/session')->setFormData(false);
