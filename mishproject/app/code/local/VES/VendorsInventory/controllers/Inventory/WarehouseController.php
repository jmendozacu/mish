...
class VES_VendorsInventory_Inventory_WarehouseController extends VES_Vendors_Controller_Action {
    ...
    protected function _initAction() {
        $this->loadLayout()
                ...
                ->_addBreadcrumb(
                        Mage::helper('vendorsinventory')->__('Manage Warehouses'), Mage::helper('vendorsinventory')->__('Manage Warehouses')
        );
        return $this;
    }
    ...
    public function indexAction() {
        if (Mage::getModel('vendors/session')->getData('warehouseaddmore_product_import'))
            Mage::getModel('vendors/session')->setData('warehouseaddmore_product_import', null);
        if (Mage::getModel('vendors/session')->getData('null_warehouseaddmore_product_import'))
            Mage::getModel('vendors/session')->setData('null_warehouseaddmore_product_import', 0);
    ...
    public function editAction() {
        ...
        if ($model->getId() || $warehouseId == 0) {
            ...
            $this->_addBreadcrumb(Mage::helper('vendorsinventory')->__('Manage Warehouses'), Mage::helper('adminhtml')->__('Manage Warehouses'));
            $this->_addBreadcrumb(Mage::helper('vendorsinventory')->__('Warehouse'), Mage::helper('adminhtml')->__('Warehouse'));

            $this->getLayout()->getBlock('head')->setCanLoadExtJs(true)
                    ->removeItem('js', 'mage/adminhtml/grid.js')
                    ->addItem('js', 'magestore/adminhtml/inventory/grid.js');
            $this->_addContent($this->getLayout()->createBlock('vendorsinventory/warehouse_edit'))
                    ->_addLeft($this->getLayout()->createBlock('vendorsinventory/warehouse_edit_tabs', 'warehouse_edit_tabs'));
            Mage::dispatchEvent('warehouse_controller_index', array('warehouse_controler' => $this));
            $this->renderLayout();
        } else {
            Mage::getSingleton('vendors/session')->addError(Mage::helper('inventoryplus')->__('Warehouse does not exist!'));
            $this->_redirect('*/*/');
        }
    }
...
//            }
            ...
            if ($vendorArray) {
                foreach ($vendorArray as $vendorId) {
                    ...
                    if ($vendorData->getId()) {
                        if (in_array($vendorId, $array)) {
                            $vendorData->setData('can_edit_warehouse', 1);
                        }
    ...
    protected function _saveHistory($model, $changeData, $changeArray, $oldManager, $manager) {
        $vendor = Mage::getModel('vendors/session')->getUser();
        $admin = $vendor->getVendorId();
        ...
        if (!$this->getRequest()->getParam('id')) {
            ...
            $warehouseHistory->setData('warehouse_id', $model->getId())
                    ...
                    ->setVendorId('vendor_id', $vendorId)
                    ->save();
...
//            $this->_checkRootWarehouse($data);
            ...
            if ($this->getRequest()->getParam('id')) {
                ...
                $model->setVendorId($admin->getId());
            }
            if (!$this->getRequest()->getParam('id')) {
                $model->setCreatedBy($admin->getVendorId());
                $model->setManager($admin->getVendorId());
                $model->setManagerName($admin->getVendorId());
                $model->setManagerEmail($admin->getEmail());
                $model->setWarehouseCreatedBy('2');
                $model->setVendorId($admin->getId());
            ...
            try {
                /* save warehouse data */
                $model->setUpdatedBy($admin->getVendorId());
                $model->save();
        ...
        Mage::getSingleton('vendors/session')->addError(Mage::helper('inventoryplus')->__('Unable to find warehouse to save!'));
        return $this->_redirect('*/*/');
    }

    protected function _prepareMessages() {
        if (!$this->getRequest()->getParam('id')) {
            Mage::getSingleton('vendors/session')->addNotice(Mage::helper('inventoryplus')->__('The warehouse is empty. You can add products by adjusting stock or requesting stock for sending stock to this warehouse.'));
            Mage::getSingleton('vendors/session')->addSuccess(
                    Mage::helper('inventoryplus')->__('The warehouse has been created.')
            );
        } else {
            Mage::getSingleton('vendors/session')->addSuccess(Mage::helper('inventoryplus')->__('The warehouse has been saved.'));
        }
    }
    ...
    public function gridAction() {
        $this->loadLayout();
        $this->getLayout()->getBlock('vendorsinventory_warehouse_grid');
        $this->renderLayout();
    }
    ...
    public function storegridAction() {
        ...
        $this->getResponse()->setBody(
            $this->getLayout()->createBlock('vendorsinventory/warehouse_edit_tab_store')
                ->toHtml()
        );
    }
    ...
    public function viewtransactionAction() {
        ...
        if ($model->getId() || $transactionId == 0) {
            ...
            $this->getLayout()->getBlock('head')->setCanLoadExtJs(true)
                    ->removeItem('js', 'mage/adminhtml/grid.js')
                    ->addItem('js', 'magestore/adminhtml/inventory/grid.js');
            $this->_addContent($this->getLayout()->createBlock('vendorsinventory/warehouse_transaction_view'))
                    ->_addLeft($this->getLayout()->createBlock('vendorsinventory/warehouse_transaction_view_edit_tabs'));
            $this->renderLayout();
        } else {
            Mage::getSingleton('vendors/session')->addError(Mage::helper('inventoryplus')->__('Warehouse transaction does not exist!'));
            $this->_redirect('*/*/');
        }
    }
}
