 ...
 */
class VES_VendorsInventory_Inventory_ProcessController extends Mage_Adminhtml_Controller_Action {
    ...
    public function doProcessAction() {
        ...
        $count = Mage::getSingleton('vendors/session')->getData('IMProcess_count' . $this->getTypeKey($type));
        $count += 174;
        Mage::getSingleton('vendors/session')->setData('IMProcess_count' . $this->getTypeKey($type), $count);
        $total = 1000;
    ...
    protected function _resetDataCount($types) {
        foreach ($types as $type) {
            Mage::getSingleton('vendors/session')->unsetData('IMProcess_count' . $this->getTypeKey($type));
        }
    }
