 ...
 */
class VES_VendorsInventory_Inventory_RequeststockController extends VES_Vendors_Controller_Action  {
    ...
    public function newAction() {
        ...
        $this->_addBreadcrumb(
                Mage::helper('vendorsinventory')->__('Stock Requesting Manager'), Mage::helper('vendorsinventory')->__('Stock Transfering Manager')
        );
        $this->_addBreadcrumb(
                Mage::helper('vendorsinventory')->__('Stock Requesting News'), Mage::helper('vendorsinventory')->__('Stock Transfering News')
        );
        $this->getLayout()->getBlock('head')->setCanLoadExtJs(true)
                ->removeItem('js', 'mage/adminhtml/grid.js')
                ->addItem('js', 'magestore/adminhtml/inventory/grid.js');
        $this->_addContent($this->getLayout()->createBlock('vendorsinventory/requeststock_edit'))
                ->_addLeft($this->getLayout()->createBlock('vendorsinventory/requeststock_edit_tabs'));
        $this->renderLayout();
    }
    ...
    public function editAction() {
        ...
        if ($model->getId() || $requeststock == 0) {
            ...
            Mage::getSingleton('adminhtml/session')->addError(
                    Mage::helper('vendorsinventory')->__('Requesting stock does not exist')
            );
            $this->_redirect('*/*/');
        }
    }
    ...
    public function productsAction() {
        ...
        $this->renderLayout();
        if (Mage::getModel('vendors/session')->getData('requeststock_product_import'))
            Mage::getModel('vendors/session')->setData('requeststock_product_import', null);
    }
    ...
    public function getImportCsvAction() {
        ...
        if (isset($fileToUpload['fileToUpload']['name']) && $fileToUpload['fileToUpload']['name'] != '') {
            try {
                Mage::getModel('vendors/session')->setData('request_stock_reason', null);
                if ($this->getRequest()->getParam('reason')) {
                    Mage::getModel('vendors/session')->setData('request_stock_reason', $this->getRequest()->getParam('reason'));
                }
                ...
                Mage::getModel('vendors/session')->setData('requeststock_product_import', null);
                Mage::getModel('vendors/session')->setData('null_requeststock_product_import', null);
                if (!empty($requeststockProducts)) {
                    Mage::getModel('vendors/session')->setData('requeststock_product_import', $requeststockProducts);
                } else {
                    Mage::getModel('vendor/session')->setData('null_requeststock_product_import', 1);
                }
            } catch (Exception $e) {
                    ...
                    try {
                        if (Mage::getStoreConfig('inventoryplus/transaction/transaction_notice', $store->getId())) {
                            ...
                            if ($source && !$warehouseSource->getIsUnwarehouse()) {
                                ...
                                Mage::helper('vendorsinventory/email')->sendSendstockEmail($warehouseSource, $model->getId(), 0, $stockName);
                            }
                        }
                    } catch (Exception $e) {
...
//                $this->removeScanData();
                ...
                if ($this->getRequest()->getParam('back')) {
                    $this->_redirect('vendors/inventory_requeststock/edit', array('id' => $model->getId()));
                    return;
                }
                $this->_redirect('*/*/');
                return;
                ...
                $this->_redirect('vendors/inventory_requeststock/edit', array('id' => $this->getRequest()->getParam('id')));
                return;
            }
            Mage::getSingleton('vendors/session')->addError(
                    Mage::helper('vendorsinventory')->__('Unable to save')
            );
            $this->_redirect('*/*/');
        } else {
            Mage::getSingleton('vendors/session')->addError(Mage::helper('inventorywarehouse')->__('Unable to save'));
            $this->_redirect('*/*/');
        }
    }
    ...
    public function cancelAction() {
        ...
        try {
            ...
            if ($this->getRequest()->getParam('warehouse_id'))
                $this->_redirect('vendors/inventory_warehouse/edit', array('id' => $this->getRequest()->getParam('warehouse_id')));
            else
                $this->_redirect('vendors/inventory_requeststock/index');
            return;
        } catch (Exception $e) {
            Mage::getSingleton('vendors/session')->addError($e->getMessage());
            if ($this->getRequest()->getParam('warehouse_id'))
        ...
        Mage::getSingleton('vendors/session')->addError(Mage::helper('vendorsinventory')->__('Unable to cancel'));
        if ($this->getRequest()->getParam('warehouse_id'))
            $this->_redirect('vendors/inventory_warehouse/edit', array('id' => $this->getRequest()->getParam('warehouse_id')));
        else
            $this->_redirect('vendors/inventory_requeststock/index');
    ...
    public function exportCsvAction() {
        ...
        $content = $this->getLayout()
                ->createBlock('vendorsinventory/requeststock_grid')
                ->getCsv();
    ...
    public function exportpdfrequeststockAction() {
        try {
            ...
            $contents .= $this->getLayout()->createBlock('vendorsinventory/sendstock')
                    ->setRequeststock($warehouse_requeststock_id)
                    ->setRequeststockproducts($requeststockProducts)
                    ->setTemplate('ves_vendorsinventory/requeststock/requeststock.phtml')
                    ->toHtml();
