...
class VES_VendorsInventory_Block_Inventorysupplyneeds_Renderer_Qtyforsupply extends Mage_Adminhtml_Block_Widget_Grid_Column_Renderer_Abstract {
    ...
    public function render(Varien_Object $row) {
        ...
        $min_needs = Mage::helper('vendorsinventory/supplyneeds')->calMin($product_id, $warehouse);        
        if ($datefrom && $dateto && $method == 2 && (strtotime($datefrom) <= strtotime($dateto))) {            
//            $max_needs = ceil($row->getTotalOrder() / 10) + $min_needs;
            $max_needs = Mage::helper('vendorsinventory/supplyneeds')->calMaxAverage($product_id, $datefrom, $dateto, $warehouse);
        } elseif ($datefrom && $dateto && $method == 1 && strtotime($datefrom) <= strtotime($dateto)) {
            $max_needs = Mage::helper('vendorsinventory/supplyneeds')->calMaxExponential($product_id, $datefrom, $dateto, $warehouse);
        } else {
            $min_needs = 0;
            $max_needs = 0;
        }
        ...
        $url = $this->getUrl('vendors/inventory_inventorysupplyneeds/chart'); //.'product_id/'.$product_id;
