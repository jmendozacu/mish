 ...
 */
class VES_VendorsInventory_Block_Physicalstocktaking_Edit extends Mage_Adminhtml_Block_Widget_Form_Container {
    ...
    public function __construct() {
        ...
        $this->_addButton('cancel', array('label' => Mage::helper('vendorsinventory')->__('Cancel'), 'class' => 'cancel', 'onclick' => 'cancelAction()'));
        $this->_addButton('confirm', array('label' => Mage::helper('vendorsinventory')->__('Confirm'), 'style' => 'comfirm', 'onclick' => 'confirmAction()'));

        $this->_addButton('confirm_adjust', array('label' => Mage::helper('vendorsinventory')->__('Confirm and Adjust Stock'), 'style' => 'comfirm', 'onclick' => 'confirmAdjustAction()'));

        $this->_updateButton('reset', 'onclick', 'setLocation(\'' . $this->getUrl('vendors/inventory_physicalstocktaking/new') . '\')');
        if ($warehouseBack = $this->getRequest()->getParam('warehouseback_id'))
            $this->_updateButton('back', 'onclick', 'setLocation(\'' . $this->getUrl('vendors/inventory_warehouse/edit/id/' . $warehouseBack) . '\')');
        ...
        if ($this->getRequest()->getParam('id')) {
            $this->_addButton('print', array(
                'name' => 'print',
                'label' => Mage::helper('adminhtml')->__('Print Product List'),
                'onclick' => 'setLocation(\'' . $this->getUrl('*/*/exportpdfstocktaking', array('_current' => true)) . '\')',
                'class' => 'save'
                    ), -50);
            $admin = Mage::getSingleton('vendors/session')->getUser();
            $adminId = $admin->getId();
            $permission = Mage::getSingleton('vendors/session')->getData('inventory_permission');
            ...
            $canPhysical = Mage::helper('vendorsinventory/adjuststock')->getPermission($warehouseId, 'can_physical');
            $canAdjust = Mage::helper('vendorsinventory/adjuststock')->getPermission($warehouseId, 'can_adjust');
            ...
            $this->_addButton('saveandprint', array(
                'name' => 'saveandprint',
                'label' => Mage::helper('adminhtml')->__('Save and Print Product List'),
                'onclick' => 'saveAndPrint()',
                'class' => 'save'
                    ), -50);
            if ($prepareData = Mage::getModel('vendors/session')->getData('physicalstocktaking_product_warehouse')) {
        ...
        $this->_formScripts[] = "
            ...
            function cancelAction(){
                var r=confirm('" . Mage::helper('vendorsinventory')->__('Are you sure you want to cancel this physical stocktaking?') . "');  
                if (r==true){
                ...
                for(var i = 0;i<adjustFields.length;i++){
                    ...
                    if(!el.disabled && el.tagName == 'INPUT'){
                        if(!el.value || el.value < 0 || !isNumeric(el.value)){
                            ...
                            var textnode = document.createTextNode('" . Mage::helper('vendorsinventory')->__("Physical Qty. does not accept negative values or blank. Please enter a valid value.") . "');
            ...
            function saveAndPrint(){
                var r=confirm('Are you sure you want to save and print this physical stocktaking?');
                if (r==true){
                    editForm.submit($('edit_form').action+'exportpdf/edit/')
                }
            }
            ...
            function confirmAction(){
                var validate = validateQty();
                if(validate){
                    var r=confirm('" . Mage::helper('vendorsinventory')->__('Are you sure you want to confirm this physical stocktaking? The comfirmation will complete the stocktaking process and create a pending stock adjustment.?') . "');
                    if (r==true){
        ...
        var reason = '';
            function uploadFile() {
                ...
                xhr.open('POST', '" . $this->getUrl('vendors/inventory_physicalstocktaking/importproduct') . "');
                xhr.send(fd);
            }
        ...
        if ($this->getRequest()->getParam('exportpdf')) {
            $this->_formScripts[] = "
                ...
                setTimeout(printStockTakingList, 2000);
                 
                function printStockTakingList() {
                    setLocation('" . $this->getUrl('*/*/exportpdfstocktaking', array('_current' => true)) . "');
                }
             ";
        }
    }
    ...
    public function getHeaderText() {
        if (Mage::registry('physicalstocktaking_data') && Mage::registry('physicalstocktaking_data')->getId()) {
            return Mage::helper('vendorsinventory')->__("View Physical Stocktaking No. '%s'", $this->htmlEscape(Mage::registry('physicalstocktaking_data')->getId()));
        }
        return Mage::helper('vendorsinventory')->__('New Physical Stocktaking');
    }
