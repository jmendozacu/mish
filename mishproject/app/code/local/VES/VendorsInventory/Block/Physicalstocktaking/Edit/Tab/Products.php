 ...
 */
class VES_VendorsInventory_Block_Physicalstocktaking_Edit_Tab_Products extends Mage_Adminhtml_Block_Widget_Grid {//implements Magestore_Inventoryplus_Block_Adminhtml_Barcode_Scan_Grid {
    ...
    public function __construct() {
        ...
        if (($this->getAdjustStock() && $this->getAdjustStock()->getId()) || Mage::getModel('vendors/session')->getData('physicalstocktaking_product_import')) {
    ...
    protected function _prepareCollection() {
        ...
        $currentAdminId = $vendorSession->getVendorId();
        ...
        if ($this->getRequest()->getParam('id')) {
            ...
            if ($adjustStockProducts = Mage::getModel('vendors/session')->getData('physicalstocktaking_product_warehouse')) {
            ...
            $collection = Mage::getModel('catalog/product')->getCollection()
                    ->addAttributeToSelect('*')
                    ->addAttributeToFilter('vendor_id', $currentAdminId)
                    ->addAttributeToFilter('type_id', array('nin' => array('configurable', 'bundle', 'grouped')));
        ...
        $collection->setPageSize($productLimit)->load();
    ...
    protected function _prepareColumns() {
        ...
        $physicalPermission = Mage::helper('vendorsinventory/physicalstock')->getPhysicalWarehouseByAdmin();
        ...
        $this->addColumn('category', array(
            ...
            'renderer' => 'vendorsinventory/physicalstocktaking_renderer_category',
    ...
    protected function _getSelectedProducts() { 
        ...
        if ((!is_array($products) || Mage::getModel('vendors/session')->getData('physicalstocktaking_product_import'))) {
    ...
    public function getSelectedRelatedProducts() { 
        $products = array();
        if ($adjustStockProducts = Mage::getModel('vendors/session')->getData('physicalstocktaking_product_import')) {
            foreach ($adjustStockProducts as $adjustStockProduct) {
            ...
            if ($this->getRequest()->getParam('id')) {
                ...
                if(Mage::helper('core')->isModuleEnabled('Magestore_Inventorybarcode')){
                    $scanItems = Mage::getModel('vendorsinventory/permission')//Mage::getModel('inventorybarcode/barcode_scanitem')
                            ->getItems(array(), VES_VendorsInventory_Block_Physicalstocktaking_Scanbarcode::SCAN_ACTION);
                    if (count($scanItems)) {
