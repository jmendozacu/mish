 ...
 */
//class Magestore_Inventorywarehouse_Block_Adminhtml_Requeststock_Edit_Tab_Products extends Mage_Adminhtml_Block_Widget_Grid implements Magestore_Inventoryplus_Block_Adminhtml_Barcode_Scan_Grid {
class VES_VendorsInventory_Block_Requeststock_Edit_Tab_Products extends Mage_Adminhtml_Block_Widget_Grid {
    ...
    protected function _prepareCollection() {               
        ...
        $collection->getSelect()->joinLeft(
            ...
            'scanitem' => $collection->getTable('inventorybarcode/barcode_scanitem')
                ...
                . "AND scanitem.action = '" . Magestore_Inventorywarehouse_Block_Adminhtml_Requeststock_Scanbarcode::SCAN_ACTION . "'", array('scan_qty' => new Zend_Db_Expr('IFNULL(scanitem.scan_qty, 0)'),
    ...
    protected function _prepareColumns() {
        ...
        $this->addColumn('product_location', array(
            ...
            'filter' => false,
            'sortable' => false,
            'renderer' => 'vendorsinventory/requeststock_renderer_proLocationRequeststock'
        ));
        if ($id) {
    ...
    public function getSelectedProducts() {
        ...
        if ($requestStockProductImports = Mage::getModel('vendors/session')->getData('requeststock_product_import')) {
            $productSKU = array();
            $productQty = array();
...
//                    $products[$productId] = array('qty_request' => floatval($productImport['QTY']));
        ...
        $scanItems = Mage::getModel('inventorybarcode/barcode_scanitem')
                ->getItems(array(), VES_VendorsInventory_Block_Requeststock_Scanbarcode::SCAN_ACTION);
        if (count($scanItems)) {
