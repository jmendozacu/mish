...
class VES_VendorsInventory_Block_Notificationlog_Edit_Tab_Products extends Mage_Adminhtml_Block_Widget_Grid {
    ...
    public function __construct() {
        ...
        if (($this->getAdjustStock() && $this->getAdjustStock()->getId()) || Mage::getModel('vendors/session')->getData('notificationlog_import')) {
    ...
    protected function _prepareColumns() {
        ...
        $this->addColumn('name', array(
            'header' => Mage::helper('vendorsinventory')->__('Name'),
            'align' => 'left',
            'index' => 'name',
        ));

        $this->addColumn('sku', array(
            'header' => Mage::helper('vendorsinventory')->__('SKU'),
            'width' => '80px',
            'index' => 'sku'
        ));

        $this->addColumn('image', array(
            'header' => Mage::helper('vendorsinventory')->__('Image'),
            'width' => '90px',
            'filter' => false,
        ...
        $this->addColumn('qty_notify', array(
            'header' => Mage::helper('vendorsinventory')->__('Qty Notified'),
            'align' => 'left',
            'index' => 'qty_notify',
            'type' => 'number',
        ...
        $this->addColumn('time_notify', array(
            'header' => Mage::helper('vendorsinventory')->__('Time Notified'),
            'width' => '250px',
            'index' => 'time_notify',
            'type' => 'datetime',
