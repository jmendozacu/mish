...
class AW_Deliverydate_Block_Adminhtml_Form_Daysselector extends Mage_Adminhtml_Block_Widget implements Varien_Data_Form_Element_Renderer_Interface{
	public function __construct(){
        $this->setTemplate('aw_deliverydate/daysselector.phtml');
    }
    ...
    public function getWebsites()
    {
        ...
        if (Mage::app()->isSingleStoreMode() || !$this->getStoreId()) {
            return $websites;
        }elseif ($storeId = $this->getStoreId()) {
	...
	public function getValues(){
		...
		foreach(Mage::getModel('deliverydate/holiday')->getCollection()
					->addStoreIdFilter($this->getStoreId()) as $i){
			$data[] = $i->getData();
		}
		return $data;
		return Mage::getModel('deliverydate/holiday')->getCollection()
